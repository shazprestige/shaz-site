...
let memberFilterState={search:'',province:'',district:'',sms:'',emailMarketing:'',provider:'',sort:'newest'};
...
function memberFiltered(){
  ...
  const out=(members||[]).filter(u=>{const hay=[u.firstName,u.lastName,u.email,u.phone,u.province,u.district,u.address].join(' ').toLocaleLowerCase('tr-TR'),providers=(u.authProviders||[]).map(x=>String(x).toLowerCase());return (!search||hay.includes(search))&&(!province||u.province===province)&&(!district||u.district===district)&&(!sms||String(!!u.smsMarketingConsent)===sms)&&(!emailMarketing||String(!!u.emailMarketingConsent)===emailMarketing)&&(!provider||providers.includes(provider))});
...
function memberRowsHtml(){return memberFiltered().map(u=>{
