...
async function sendPwaIcon(req,res,size,ratio=.70){
  try{
    const a=brandAsset();if(a.remote)return res.redirect(302,a.remote);if(!a.file)return res.status(404).end();
    const safe=Math.max(1,Math.round(size*ratio));
    const logo=await sharp(a.file).resize(safe,safe,{fit:'inside',withoutEnlargement:false}).png().toBuffer();
...
for(const [name,fallback] of Object.entries({
  'users.json':[],'customers.json':[],'addresses.json':[],'favorites.json':[],'marketing_consents.json':[],'legal_acceptances.json':[],'phone_verifications.json':[],'password_resets.json':[],'new_member_coupon_templates.json':[],'account_login_attempts.json':{},'push_subscriptions.json':[],'legal_documents_backup.json':[],
...
function adminMemberRows(){
  ...
  return users.map(u=>{
    ...
    const ua=acts.filter(x=>x.userId===u.id||x.customerId===u.customerId),visits=ua.flatMap(x=>x.visits||[]).sort();return {id:u.id,customerId:u.customerId,firstName:u.firstName||'',lastName:u.lastName||'',email:u.email||'',phone:normalizeAccountPhone(u.phone)||u.phone||'',extraPhone:primary.extraPhone||'',lastSeenAt:visits.at(-1)||'',visitCount:visits.length,visits,birthDate:u.birthDate||'',phoneVerifiedAt:u.phoneVerifiedAt||null,smsMarketingConsent:!!u.smsMarketingConsent,emailMarketingConsent:!!u.emailMarketingConsent,disabled:!!u.disabled,disabledAt:u.disabledAt||null,authProviders:Array.isArray(u.authProviders)?u.authProviders:(u.passwordHash?['password']:[]),createdAt:u.createdAt||'',addressCount:userAddresses.length,orderCount:orders.filter(o=>o.userId===u.id||o.customerId===u.customerId).length,couponCount:userCoupons.length,coupons:userCoupons,profileChangeHistory:Array.isArray(u.profileChangeHistory)?u.profileChangeHistory:[],province:primary.province||'',district:primary.district||'',address:[primary.neighborhood,primary.avenue,primary.street,primary.fullAddress,primary.buildingNo?`Bina ${primary.buildingNo}`:'',primary.floor?`Kat ${primary.floor}`:'',primary.doorNo?`Daire ${primary.doorNo}`:''].filter(Boolean).join(' · '),addresses:userAddresses};
  });
}
...
app.post('/api/admin/push/send',sameOriginGuard,requireAdmin,async(req,res)=>{if(!webPushReady)return res.status(503).json({ok:false,message:'VAPID anahtarları yapılandırılmamış.'});const title=String(req.body.title||'SHAZ').trim().slice(0,80)||'SHAZ',body=String(req.body.body||'').trim().slice(0,240),tag=String(req.body.tag||'').trim().slice(0,80),rawUrl=String(req.body.url||'/').trim();let url='/';try{const u=new URL(rawUrl,SHAZ_ORIGIN);if(u.origin===SHAZ_ORIGIN)url=u.pathname+u.search+u.hash}catch{}const result=await sendPushRows(readJson('push_subscriptions.json',[]),{title,body,icon:'/icon-192.png',badge:'/icon-192.png',url,tag,data:{url}});res.json({ok:true,...result})});
