...
for(const name of ['settings.json','catalog.json','orders.json','users.json','customers.json','addresses.json','favorites.json','marketing_consents.json','legal_documents.json','legal_documents_backup.json','legal_acceptances.json','phone_verifications.json','password_resets.json','coupons.json','account_state.enc']){
...
for(const [name,fallback] of Object.entries({
  'users.json':[],'customers.json':[],'addresses.json':[],'favorites.json':[],'marketing_consents.json':[],'legal_acceptances.json':[],'phone_verifications.json':[],'password_resets.json':[],'legal_documents_backup.json':[],
...
const ACCOUNT_LOGIN_MAX_ATTEMPTS=5,ACCOUNT_LOGIN_WINDOW_MS=15*60*1000;
function accountLoginAttemptKey(req,login=''){const id=String(login||'').trim().toLocaleLowerCase('tr-TR');return `${accountRateKey(req)}|${id}`}
function accountAttemptState(key){const now=Date.now(),x=accountLoginAttempts.get(key);if(!x||now-x.start>ACCOUNT_LOGIN_WINDOW_MS){const fresh={start:now,count:0};accountLoginAttempts.set(key,fresh);return fresh}return x}
function accountFailKey(key){const x=accountAttemptState(key);x.count+=1;accountLoginAttempts.set(key,x);return Math.max(0,ACCOUNT_LOGIN_MAX_ATTEMPTS-x.count)}
function accountBlockedMessage(key){const x=accountAttemptState(key),remainingMs=Math.max(0,ACCOUNT_LOGIN_WINDOW_MS-(Date.now()-x.start)),mins=Math.max(1,Math.ceil(remainingMs/60000));return `5 hatalı giriş hakkınız doldu. ${mins} dakika sonra tekrar deneyin.`}
...
const ACCOUNT_SNAPSHOT_NAMES=['users.json','customers.json','addresses.json','favorites.json','marketing_consents.json','phone_verifications.json','password_resets.json','coupons.json'];
...
function normalizeAdminCouponInput(body,current={}){
  ...
  const value=Number(body.value??current.value??0),expiresAt=String((body.expiresAt ?? couponExpiryDateKey(current.expiresAt) ?? '')).trim(),title=String(body.title??current.title??'').trim();
  ...
  return {code,discountType,value,expiresAt,title,minCartAmount,stackable,maxDiscountAmount};
}
