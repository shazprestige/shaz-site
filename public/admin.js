...
let memberFilterState={search:'',province:'',district:'',sms:'',emailMarketing:'',provider:'',dateFrom:'',dateTo:'',sort:'newest'};
...
function memberFiltered(){
  const search=memberFilterState.search.trim().toLocaleLowerCase('tr-TR'),province=memberFilterState.province,district=memberFilterState.district,sms=memberFilterState.sms,emailMarketing=memberFilterState.emailMarketing,provider=memberFilterState.provider;
  ...
  const out=(members||[]).filter(u=>{const hay=[u.firstName,u.lastName,u.email,u.phone,u.province,u.district,u.address].join(' ').toLocaleLowerCase('tr-TR'),providers=(u.authProviders||[]).map(x=>String(x).toLowerCase()),createdTs=new Date(u.createdAt||0).getTime();return (!search||hay.includes(search))&&(!province||u.province===province)&&(!district||u.district===district)&&(!sms||String(!!u.smsMarketingConsent)===sms)&&(!emailMarketing||String(!!u.emailMarketingConsent)===emailMarketing)&&(!provider||providers.includes(provider))&&(!fromTs||createdTs>=fromTs)&&(!toTs||createdTs<=toTs)});
  out.sort((a,b)=>memberFilterState.sort==='name'?`${a.firstName} ${a.lastName}`.localeCompare(`${b.firstName} ${b.lastName}`,'tr'):memberFilterState.sort==='orders'?Number(b.orderCount||0)-Number(a.orderCount||0):memberFilterState.sort==='province'?`${a.province} ${a.district}`.localeCompare(`${b.province} ${b.district}`,'tr'):new Date(b.createdAt||0)-new Date(a.createdAt||0));
  return out;
}
function memberRowsHtml(){const filtered=memberFiltered();return filtered.map((u,index)=>{
  ...
  return `<details class="memberAdminRow"><summary><span class="memberAdminPick"><span class="memberAdminIndex">${index+1}</span><label class="memberSelectBox" onclick="event.stopPropagation()"><input type="checkbox" ${checked} onchange="toggleMemberSelection('${attr(u.id)}',this.checked)"><span></span></label></span><span class="memberAdminName"><b>${esc([u.firstName,u.lastName].filter(Boolean).join(' ')||'İsimsiz Üye')}</b><small>${esc(u.email||'')}</small></span><strong>${esc(u.phone||'Telefon yok')}</strong><span class="memberAdminLocation"><b>${esc([u.province,u.district].filter(Boolean).join(' / ')||'Adres yok')}</b><small>${esc(u.address||'')}</small></span><span class="memberAdminOrders">${Number(u.orderCount||0)} sipariş</span><span class="memberAdminArrow">⌄</span></summary><div class="memberAdminDetail"><div><span>E-posta</span><b>${esc(u.email||'—')}</b></div><div><span>Telefon</span><b>${esc(u.phone||'—')}</b></div><div><span>Doğum Tarihi</span><b>${esc(u.birthDate||'—')}</b></div><div><span>Üyelik Tarihi</span><b>${esc(formatAdminDate(u.createdAt))}</b></div><div><span>Telefon Doğrulama</span><b>${u.phoneVerifiedAt?'Doğrulandı':'Doğrulanmadı'}</b></div><div><span>SMS İzni</span><b>${u.smsMarketingConsent?'Açık':'Kapalı'}</b></div><div><span>E-posta İzni</span><b>${u.emailMarketingConsent?'Açık':'Kapalı'}</b></div><div><span>Giriş Yöntemi</span><b>${esc((u.authProviders||['password']).join(', '))}</b></div><div><span>Kayıtlı Adres</span><b>${Number(u.addressCount||0)}</b></div><div><span>Toplam Sipariş</span><b>${Number(u.orderCount||0)}</b></div><div class="memberAllAddresses"><span>Tüm Adres Bilgileri</span>${addresses}</div></div></details>`
...
function openMemberCouponModal(){if(!selectedMemberIds.size)return alert('Önce kupon tanımlamak istediğin üyeleri seç.');document.querySelector('.memberCouponOverlay')?.remove();const el=document.createElement('div');el.className='memberCouponOverlay';el.innerHTML=`<div class="memberCouponDialog"><div class="memberCouponHead"><div><h2>Kupon Tanımla</h2><p><b>${selectedMemberIds.size}</b> seçili üyeye aynı kupon tanımlanacak.</p></div><button class="smallBtn" onclick="closeMemberCouponModal()">Kapat</button></div><div class="memberCouponGrid"><div class="field"><label><b>Kupon Kodu *</b></label><input id="memberCouponCode" class="formControl" maxlength="32" placeholder="Örn: SHAZ100"></div><div class="field"><label><b>İndirim Türü *</b></label><select id="memberCouponType" class="formControl"><option value="percent">Yüzde (%)</option><option value="fixed">Sabit TL</option></select></div><div class="field"><label><b>İndirim Değeri *</b></label><input id="memberCouponValue" class="formControl" type="number" min="1" step="1" placeholder="Örn: 10"></div><div class="field"><label><b>Son Kullanma Tarihi</b></label><input id="memberCouponExpiry" class="formControl" type="date"></div><div class="field memberCouponWide"><label><b>Açıklama</b></label><input id="memberCouponTitle" class="formControl" placeholder="Örn: Üyeye özel kupon"></div></div><button class="btn" onclick="assignCouponToMembers(this)">Seçili Üyelere Kupon Tanımla</button></div>`;document.body.appendChild(el);el.addEventListener('click',e=>{if(e.target===el)el.remove()})}
function closeMemberCouponModal(){document.querySelector('.memberCouponOverlay')?.remove()}
async function assignCouponToMembers(btn){const body={userIds:[...selectedMemberIds],code:$('#memberCouponCode')?.value,discountType:$('#memberCouponType')?.value,value:Number($('#memberCouponValue')?.value||0),expiresAt:$('#memberCouponExpiry')?.value,title:$('#memberCouponTitle')?.value};try{btn.disabled=true;const r=await fetch('/api/admin/users/coupons',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Kupon tanımlanamadı.');alert(`${r.assigned} üyeye ${r.code} kuponu tanımlandı.`);selectedMemberIds.clear();closeMemberCouponModal();updateMemberRows()}catch(e){alert(e.message)}finally{btn.disabled=false}}
