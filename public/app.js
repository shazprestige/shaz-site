...
function accountBrandPane(){return `<div class="accountBrandPane"><img src="${escapeAttr(accountLogoUrl())}" alt="SHAZ"><div class="accountBrandWord">SHAZ</div></div>`}
function accountMobileBar(backAction='closeDrawer()'){return `<div class="accountMobileBar"><button type="button" class="accountMobileBack" onclick="${backAction}">←</button><img src="${escapeAttr(accountLogoUrl())}" alt="SHAZ"><button type="button" class="accountMobileClose" onclick="closeDrawer()">×</button></div>`}
function accountAuthPage(title,subtitle,body,backAction='showAuthChoice()'){return `<div class="accountPageShell authAccountPage">${accountBrandPane()}<section class="accountPagePanel">${accountMobileBar(backAction)}<div class="authCenter"><h1>${title}</h1><p class="authLead">${subtitle}</p>${body}</div></section></div>`}
...
function showLogin(){closeAccountPopover();const body=`<div class="authForm kigiliAuthForm"><input id="loginEmail" class="formControl" type="text" autocomplete="username" placeholder="E-posta adresi veya telefon">${authPasswordField('loginPassword','Şifre','current-password')}<button class="btn authSubmit" onclick="loginAccount(this)">OTURUM AÇ</button></div><button class="authTextLink authForgot" type="button" onclick="showForgotPassword()">ŞİFREMİ UNUTTUM</button><div class="authSwitch">Hesabınız yok mu? <button type="button" onclick="showRegister()">HESAP OLUŞTUR</button></div>${socialAuthButtons()}`;openDrawer(accountAuthPage('Giriş yap','Hesabınız varsa lütfen giriş yapın.',body))}
...
function showRegister(){closeAccountPopover();const body=`<div class="authForm kigiliAuthForm"><div class="authPair"><input id="regFirst" class="formControl" placeholder="Ad"><input id="regLast" class="formControl" placeholder="Soyad"></div><input id="regEmail" class="formControl" type="email" autocomplete="email" placeholder="E-posta adresi"><input id="regPhone" class="formControl" type="tel" inputmode="tel" autocomplete="tel" placeholder="Telefon Numarası">${authPasswordField('regPass','Şifre','new-password')}<input id="regPass2" class="formControl" type="password" autocomplete="new-password" placeholder="Şifre Tekrar"><div class="birthDateField"><label for="regBirth">Doğum Tarihi</label><input id="regBirth" class="formControl" type="text" inputmode="numeric" maxlength="10" placeholder="gg.aa.yyyy"></div><div class="authInfoLine">ⓘ 6698 sayılı Kişisel Verilerin Korunması Kanunu kapsamında <button type="button" onclick="openLegalDocument('KVKK')">Üye Aydınlatma Metni</button>'mize buradan ulaşabilirsiniz.</div><div class="authChecks"><label class="authCheck"><input id="regMail" type="checkbox"><span>Kampanya, indirim ve fırsatlardan <b>E-posta</b> aracılığıyla haberdar olmak istiyorum.</span></label><label class="authCheck"><input id="regSms" type="checkbox"><span>Kampanya, indirim ve fırsatlardan <b>SMS</b> aracılığıyla haberdar olmak istiyorum.</span></label></div><div class="authLegalLinks"><button type="button" onclick="openLegalDocument('MEMBERSHIP')">Üyelik Sözleşmesi</button><button type="button" onclick="openLegalDocument('PRIVACY')">Gizlilik Politikası</button><button type="button" onclick="openLegalDocument('COOKIE')">Çerez Politikası</button></div><button class="btn authSubmit" onclick="registerAccount(this)">HESAP OLUŞTUR</button></div>${socialAuthButtons()}<div class="authSwitch">Zaten hesabınız var mı? <button type="button" onclick="showLogin()">GİRİŞ YAP</button></div>`;openDrawer(accountAuthPage('Hesap oluştur','Devam etmek için bilgilerinizi girin. Zaten bir hesabınız varsa, lütfen giriş yapın.',body));loadPublicLegalDocuments()}
async function registerAccount(btn){const p=$('#regPass')?.value||'',p2=$('#regPass2')?.value||'',birth=birthDateToIso($('#regBirth')?.value);if(p!==p2)return alert('Şifreler eşleşmiyor.');if(birth===null)return alert('Doğum tarihini gg.aa.yyyy şeklinde girin.');try{btn.disabled=true;const r=await apiJson('/api/auth/register',{method:'POST',body:JSON.stringify({firstName:$('#regFirst')?.value,lastName:$('#regLast')?.value,email:$('#regEmail')?.value,phone:$('#regPhone')?.value,password:p,birthDate:birth,smsMarketingConsent:!!$('#regSms')?.checked,emailMarketingConsent:!!$('#regMail')?.checked})});currentAccountUser=r.user;await loadAccountState();showAccountHome()}catch(e){alert(e.message)}finally{btn.disabled=false}}
function showForgotPassword(){const body=`<div class="authForm kigiliAuthForm"><input id="forgotEmail" class="formControl" type="email" autocomplete="email" placeholder="E-posta adresi"><button class="btn authSubmit" onclick="requestPasswordReset(this)">ŞİFRE SIFIRLAMA BAĞLANTISI GÖNDER</button></div><button class="authTextLink" type="button" onclick="showLogin()">GİRİŞ EKRANINA DÖN</button>`;openDrawer(accountAuthPage('Şifremi unuttum','Hesabınıza kayıtlı e-posta adresini girin.',body,'showLogin()'))}
async function requestPasswordReset(btn){try{btn.disabled=true;const r=await apiJson('/api/auth/forgot-password',{method:'POST',body:JSON.stringify({email:$('#forgotEmail')?.value})});alert(r.message||'İşlem tamamlandı.')}catch(e){alert(e.message)}finally{btn.disabled=false}}
function showResetPassword(){const token=new URLSearchParams(location.search).get('token')||'';const body=`<div class="authForm kigiliAuthForm">${authPasswordField('resetPass','Yeni Şifre','new-password')}<input id="resetPass2" class="formControl" type="password" autocomplete="new-password" placeholder="Yeni Şifre Tekrar"><button class="btn authSubmit" onclick="finishPasswordReset(this,'${escapeAttr(token)}')">ŞİFREMİ GÜNCELLE</button></div>`;openDrawer(accountAuthPage('Yeni şifre oluştur','Yeni şifrenizi belirleyin.',body,'showLogin()'))}
async function finishPasswordReset(btn,token){const a=$('#resetPass')?.value||'',b=$('#resetPass2')?.value||'';if(a!==b)return alert('Şifreler eşleşmiyor.');try{btn.disabled=true;await apiJson('/api/auth/reset-password',{method:'POST',body:JSON.stringify({token,password:a})});alert('Şifreniz güncellendi.');history.replaceState({},'', '/giris');showLogin()}catch(e){alert(e.message)}finally{btn.disabled=false}}
...
function loadExternalScript(src,id){return new Promise((resolve,reject)=>{if(document.getElementById(id))return resolve();const s=document.createElement('script');s.id=id;s.src=src;s.async=true;s.onload=resolve;s.onerror=()=>reject(new Error('Bağlantı yüklenemedi.'));document.head.appendChild(s)})}
async function startGoogleLogin(btn){try{btn.disabled=true;const c=await getSocialConfig();if(!c.googleClientId)throw new Error('Google ile giriş için GOOGLE_CLIENT_ID ayarı yapılmamış.');await loadExternalScript('https://accounts.google.com/gsi/client','shaz-google-gsi');google.accounts.id.initialize({client_id:c.googleClientId,callback:async r=>{try{const out=await apiJson('/api/auth/social/google',{method:'POST',body:JSON.stringify({credential:r.credential})});currentAccountUser=out.user;await loadAccountState();showAccountHome()}catch(e){alert(e.message)}}});google.accounts.id.prompt()}catch(e){alert(e.message)}finally{btn.disabled=false}}
async function startAppleLogin(btn){try{btn.disabled=true;const c=await getSocialConfig();if(!c.appleClientId||!c.appleRedirectUri)throw new Error('Apple ile giriş ayarları henüz tamamlanmamış.');await loadExternalScript('https://appleid.cdn-apple.com/appleauth/static/jsapi/appleid/1/en_US/appleid.auth.js','shaz-apple-auth');AppleID.auth.init({clientId:c.appleClientId,scope:'name email',redirectURI:c.appleRedirectUri,usePopup:true});const r=await AppleID.auth.signIn();const out=await apiJson('/api/auth/social/apple',{method:'POST',body:JSON.stringify({identityToken:r?.authorization?.id_token,firstName:r?.user?.name?.firstName||'',lastName:r?.user?.name?.lastName||''})});currentAccountUser=out.user;await loadAccountState();showAccountHome()}catch(e){if(String(e?.error||'')!=='popup_closed_by_user')alert(e.message||'Apple ile giriş tamamlanamadı.')}finally{btn.disabled=false}}
