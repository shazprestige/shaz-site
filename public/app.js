    ...
    <div class="checkoutScrollBody addressScrollBody">
      <div class="addressCompact addressCompactV114">
        ...
        <div class="addressPair phonePair fieldWide">
          <div class="field"><label><b>Telefon *</b><small class="fieldHelp phoneHelp">05xx xxx xx xx veya<br>5xx xxx xx xx</small></label><input class=formControl id=addr-phone type=tel inputmode=numeric autocomplete=tel value="${escapeAttr(c.phone||'')}" placeholder="05xx xxx xx xx" onpointerup="ensureAddressFieldFocus(this)" ${currentAccountUser&&checkoutState.manualAddressFromSaved?`data-address-suggest="phone" onfocus="queueSavedAddressSuggestions(this,'phone')" onclick="queueSavedAddressSuggestions(this,'phone')" oninput="closeSavedAddressSuggestions()"`:''}></div>
          <div class="field"><label><b>2. Telefon Numarası *</b><small class="fieldHelp">Size ulaşamamamız halinde kullanabileceğimiz ikinci numara zorunludur.</small></label><input class=formControl id=addr-extraPhone type=tel inputmode=numeric autocomplete=tel value="${escapeAttr(c.extraPhone||'')}" placeholder="05xx xxx xx xx" onpointerup="ensureAddressFieldFocus(this)" ${currentAccountUser&&checkoutState.manualAddressFromSaved?`data-address-suggest="extraPhone" onfocus="queueSavedAddressSuggestions(this,'extraPhone')" onclick="queueSavedAddressSuggestions(this,'extraPhone')" oninput="closeSavedAddressSuggestions()"`:''}></div>
        </div>
        ...
        <div id="homeAddressFields" class="addressModeFields fieldWide addressModeFieldsV114" style="${branch?'display:none':''}">
          ...
          <div class="field fieldWide"><label><b>Adres devamı *</b><small class="fieldHelp">Bina no, kat, daire ve gerekiyorsa site/apartman adını yazın.</small></label><textarea class=formControl id=addr-fullAddress rows=2 placeholder="Örn: No:12 Kat:2 Daire:5" onpointerdown="ensureAddressFieldFocus(this)" ${currentAccountUser&&checkoutState.manualAddressFromSaved?`data-address-suggest="fullAddress" onfocus="queueSavedAddressSuggestions(this,'fullAddress')" onclick="queueSavedAddressSuggestions(this,'fullAddress')" oninput="updateSavedAddressSuggestions(this,'fullAddress')"`:''}>${escapeHtml(c.fullAddress||'')}</textarea></div>
        </div>
...
function addrInputInner(label,id,value='',type='text',placeholder=''){
  ...
  return `<label><b>${label}</b></label><input class=formControl id="addr-${id}" type="${type}" value="${escapeAttr(value||'')}" onpointerdown="ensureAddressFieldFocus(this)" ${placeholder?`placeholder="${escapeAttr(placeholder)}"`:''}${suggest}>`;
}
...
function authPasswordField(id,placeholder,autocomplete,watchMode=''){const watch=watchMode?`;updateAuthSubmitState('${watchMode}')`:'';const coarse=!!window.matchMedia?.('(pointer:coarse)').matches,mode=coarse?'type="text" data-password-mask="css" class="formControl authCssPasswordMask"':'type="password" class="formControl"';return `<div class="authPasswordWrap"><input id="${id}" ${mode} autocomplete="${autocomplete}" placeholder="${placeholder}" onpointerdown="ensureDirectInputFocus(this)" oninput="flashPasswordChar(this,event)${watch}"><span class="authLastCharFlash" aria-hidden="true"><span class="authLastCharText"></span><i class="authFlashCaret"></i></span><button type="button" tabindex="-1" class="authEye" ontouchstart="handleAuthEyePointer(event,'${id}',this)" onmousedown="handleAuthEyePointer(event,'${id}',this)" onclick="event.preventDefault();event.stopPropagation()" aria-label="Şifreyi göster">${authEyeSvg(false)}</button></div>`}
function handleAuthEyePointer(e,id,btn){e.preventDefault();e.stopPropagation();const now=Date.now();if(btn._shazEyeToggleAt&&now-btn._shazEyeToggleAt<220)return;btn._shazEyeToggleAt=now;const x=document.getElementById(id);if(!x)return;toggleAuthPassword(id,btn);requestAnimationFrame(()=>{try{x.focus({preventScroll:true})}catch(_){try{x.focus()}catch(__){}}})}
...
function handleMarketingConsentToggle(el,channel){const was=channel==='sms'?!!currentAccountUser?.smsMarketingConsent:!!currentAccountUser?.emailMarketingConsent;if(!was||el.checked){delete el.dataset.optoutConfirmed;return}el.checked=true;const text=channel==='sms'?'SMS iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları SMS aracılığıyla alamazsınız.':'E-posta iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları e-posta aracılığıyla alamazsınız.';pendingMarketingConsent={el,channel};profileConfirmOverlay('İletişim izni değişikliği',[text],()=>{if(pendingMarketingConsent?.el===el){el.checked=false;el.indeterminate=false;el.dataset.optoutConfirmed='1';requestAnimationFrame(()=>{el.checked=false})}pendingMarketingConsent=null})}
...
function accountAddressField(label,id,value='',type='text',placeholder=''){return `<label class="accountAddressEditField"><span>${escapeHtml(label)}</span><input id="${id}" class="formControl" type="${type}" ${type==='tel'?'inputmode="tel"':''} value="${escapeAttr(value||'')}" placeholder="${escapeAttr(placeholder||label)}" onpointerdown="ensureDirectInputFocus(this)"></label>`}
function showAddressForm(id=''){const a=currentAccountAddresses.find(x=>x.id===id)||{},fullName=a.fullName||[currentAccountUser.firstName,currentAccountUser.lastName].filter(Boolean).join(' '),phone=displayAccountPhone(a.phone||currentAccountUser.phone||'');openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountAddresses()">← Geri</button><h2>${id?'Adresi Düzenle':'Yeni Adres'}</h2></div><div class="accountForm accountAddressEditForm">${accountAddressField('Adres Başlığı','aaTitle',a.title||'')}${accountAddressField('Ad Soyad','aaFullName',fullName)}<label class="accountAddressEditField"><span>1. Telefon Numarası</span><input id="aaPhone" class="formControl" type="tel" inputmode="tel" onpointerdown="ensureDirectInputFocus(this)" value="${escapeAttr(phone)}" placeholder="05xx xxx xx xx"><small>SMS ve bilgilendirmelerin geleceği numara</small></label><label class="accountAddressEditField"><span>2. Telefon Numarası (isteğe bağlı)</span><input id="aaExtraPhone" class="formControl" type="tel" inputmode="tel" onpointerdown="ensureDirectInputFocus(this)" value="${escapeAttr(displayAccountPhone(a.extraPhone||''))}" placeholder="05xx xxx xx xx"><small>SMS gönderilmez · Yedek numara</small></label><div class="authPair">${accountAddressField('İl','aaProvince',a.province||'')}${accountAddressField('İlçe','aaDistrict',a.district||'')}</div>${accountAddressField('Mahalle','aaNeighborhood',stripAddressSuffix(a.neighborhood||'','neighborhood'))}<div class="authPair">${accountAddressField('Cadde','aaAvenue',stripAddressSuffix(a.avenue||'','avenue'))}${accountAddressField('Sokak','aaStreet',stripAddressSuffix(a.street||'','street'))}</div><label class="accountAddressEditField"><span>Adres Devamı</span><textarea id="aaFullAddress" class="formControl" onpointerdown="ensureDirectInputFocus(this)" placeholder="Bina no, kat, daire, site/apartman adı">${escapeHtml(a.fullAddress||'')}</textarea></label><button class="btn" onclick="saveAccountAddress('${escapeAttr(id)}',this)">Kaydet</button></div></div>`,'showAccountAddresses()'))}
