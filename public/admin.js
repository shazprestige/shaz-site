...
function memberFiltered(){
  ...
  out.sort((a,b)=>memberFilterState.sort==='name'?`${a.firstName} ${a.lastName}`.localeCompare(`${b.firstName} ${b.lastName}`,'tr'):memberFilterState.sort==='orders'?Number(b.orderCount||0)-Number(a.orderCount||0):memberFilterState.sort==='coupons'?Number(b.couponCount||0)-Number(a.couponCount||0):memberFilterState.sort==='province'?`${a.province} ${a.district}`.localeCompare(`${b.province} ${b.district}`,'tr'):new Date(b.createdAt||0)-new Date(a.createdAt||0));
  return out;
}
...
async function sendManualPush(){const r=await fetch('/api/admin/push/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:$('#manualPushTitle').value,body:$('#manualPushBody').value,url:$('#manualPushUrl').value})}).then(x=>x.json());alert(r.ok?`${r.sent||0} cihaza gönderildi.`:(r.message||'Gönderilemedi.'))}
