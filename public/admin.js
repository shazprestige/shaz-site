...
function applyMemberDateRange(){const from=$('#memberRangeFrom')?.value||'',to=$('#memberRangeTo')?.value||'';if(from&&to&&from>to)return alert('Başlangıç tarihi bitiş tarihinden sonra olamaz.');memberFilterState.dateFrom=from;memberFilterState.dateTo=to;closeMemberDateRangePicker();renderMembers(false)}
function clearMemberDateRange(){memberFilterState.dateFrom='';memberFilterState.dateTo='';closeMemberDateRangePicker();renderMembers(false)}
