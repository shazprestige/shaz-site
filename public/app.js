...
function couponCartResult(baseTotal,ids=checkoutState.appliedCouponIds){
  const selected=[];let remaining=Math.max(0,Number(baseTotal||0)),discount=0;
  for(const id of (ids||[])){
    const c=currentAccountCoupons.find(x=>String(x.id)===String(id)&&x.status==='active');if(!c)continue;
    if(Number(c.minCartAmount||0)>remaining+discount)continue;
    ...
    amount=Math.max(0,Math.min(remaining,Math.round(amount*100)/100));remaining=Math.max(0,Math.round((remaining-amount)*100)/100);discount=Math.round((discount+amount)*100)/100;selected.push({...c,discount:amount})
  }
...
function toggleCheckoutCoupon(id){
  ...
  if(pos>=0){ids.splice(pos,1);checkoutState.appliedCouponIds=ids;return checkout(document.querySelector('.checkoutScrollBody')?.scrollTop||0)}
  if(campaign.total<Number(c.minCartAmount||0))return checkoutCouponMessage(`Bu kupon için minimum sepet tutarı ${money(c.minCartAmount)}.`);
  const selected=ids.map(cid=>currentAccountCoupons.find(x=>String(x.id)===String(cid)&&x.status==='active')).filter(Boolean),candidate=[...selected,c];
  if(candidate.length>1&&candidate.some(x=>x.stackable!==true))return checkoutCouponMessage('Bu kupon diğer seçili kuponlarla birlikte kullanılamaz.');
  ids.push(c.id);checkoutState.appliedCouponIds=ids;checkout(document.querySelector('.checkoutScrollBody')?.scrollTop||0)
}
function checkoutCouponsHtml(baseTotal){
  if(!currentAccountUser)return '';
  const rows=activeAccountCoupons();if(!rows.length)return '';
  const selected=new Set((checkoutState.appliedCouponIds||[]).map(String));
  return `<section class="checkoutCouponPanel"><div class="checkoutCouponPanelHead"><b>Kuponlarım</b><small>Uygun kuponlarınızı sepetinize uygulayabilirsiniz.</small></div><div class="checkoutCouponChoices">${rows.map(c=>{const applied=selected.has(String(c.id)),ex=couponExpiryInfo(c),min=Number(c.minCartAmount||0);return `<div class="checkoutCouponChoice ${applied?'is-applied':''}"><div><strong>${escapeHtml(couponDiscountLabel(c))}</strong><b>${escapeHtml(c.code||'')}</b>${String(c.title||'').trim()?`<small>${escapeHtml(c.title)}</small>`:''}${min?`<small>Minimum sepet: ${money(min)}</small>`:''}${c.discountType==='percent'&&Number(c.maxDiscountAmount||0)>0?`<small>Maksimum indirim: ${money(c.maxDiscountAmount)}</small>`:''}<small>${c.stackable?'Diğer uygun kuponlarla birlikte kullanılabilir':'Tek başına kullanılabilir'}</small><small class="${ex.urgent?'couponExpiryUrgent':''}">${escapeHtml(ex.text)}</small></div><button type="button" onclick="toggleCheckoutCoupon('${escapeAttr(c.id)}')">${applied?'Kaldır':'Uygula'}</button></div>`}).join('')}</div></section>`
}
...
function removeCartItem(i){
  ...
  cart.splice(i,1);if(!cart.length)checkoutState.appliedCouponIds=[];updateCart();
...
function addrInputInner(label,id,value='',type='text',placeholder=''){
  const suggest=currentAccountUser&&checkoutState.manualAddressFromSaved?` data-address-suggest="${escapeAttr(id)}" onfocus="openSavedAddressSuggestions(this,'${escapeAttr(id)}')" oninput="openSavedAddressSuggestions(this,'${escapeAttr(id)}')"`:'';
...
function closeSavedAddressSuggestions(){document.querySelector('.savedAddressSuggestionMenu')?.remove()}
function openSavedAddressSuggestions(input,field){closeSavedAddressSuggestions();if(!input||!checkoutState.manualAddressFromSaved)return;const q=String(input.value||'').trim().toLocaleLowerCase('tr-TR'),values=addressSuggestionValues(field).filter(v=>!q||v.toLocaleLowerCase('tr-TR').includes(q));if(!values.length)return;const box=document.createElement('div');box.className='savedAddressSuggestionMenu';box.innerHTML=values.map(v=>`<button type="button" onpointerdown="event.preventDefault()" data-value="${escapeAttr(v)}">${escapeHtml(v)}</button>`).join('');const r=input.getBoundingClientRect();box.style.left=r.left+'px';box.style.top=Math.min(window.innerHeight-80,r.bottom+4)+'px';box.style.width=r.width+'px';box.querySelectorAll('button').forEach(b=>b.onclick=()=>{input.value=b.dataset.value||'';closeSavedAddressSuggestions();input.focus({preventScroll:true})});document.body.appendChild(box)}
...
async function toggleFavoritesEntry(){const pop=document.querySelector('.accountEntryPopover');if(pop){pop.remove();return}const drawer=$('#drawer');if(drawer&&!drawer.classList.contains('hidden')&&drawer.dataset.view==='favorites-main'){closeDrawer();return}if(!accountStateReady)await ensureAccountStateReady();showFavorites()}
...
async function showAccountAddresses(updateRoute=true){
  ...
  try{
    ...
    openDrawer(accountStandardPage(`<div class="accountShell accountAddressesPage accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Adreslerim</h2></div>${rows||'<div class="accountEmptyState">Kayıtlı adresiniz yok.</div>'}<div class="accountAddressesToolbar"><button class="btn" onclick="showAddressForm('')">+ Yeni Adres</button></div></div>`,'showAccountHome()'))
...
showFavorites=function(){if(!currentAccountUser)return showAuthChoice();const ps=catalog.products.filter(p=>favorites.has(p.id)),content=favoriteAccountCardsHtml(ps,'favorites');openDrawer(accountStandardPage(`<div class="accountShell memberFavoritesPage accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="closeDrawer()">← Ana Sayfa</button><h2>Favorilerim</h2></div>${content}</div>`,'closeDrawer()'));const d=$('#drawer');if(d)d.dataset.view='favorites-main'};
