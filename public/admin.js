...
function memberRowsHtml(){const filtered=memberFiltered();return filtered.map((u,index)=>{
  const addresses=(u.addresses||[]).map(a=>`<div class="memberAddressBlock"><b>${esc(a.title||'Adres')}${a.isDefault?' · Varsayılan':''}</b><span>${esc(a.fullName||'')}</span><span>${esc(a.phone||'')}</span><span>${esc([a.province,a.district,a.neighborhood,a.avenue,a.street,a.fullAddress].filter(Boolean).join(' · '))}</span>${a.buildingNo||a.floor||a.doorNo?`<span>Bina: ${esc(a.buildingNo||'—')} · Kat: ${esc(a.floor||'—')} · Daire: ${esc(a.doorNo||'—')}</span>`:''}</div>`).join('')||'<div class="memberAddressBlock">Kayıtlı adres yok.</div>';
  const checked=selectedMemberIds.has(u.id)?'checked':'';
  const joined=formatAdminDate(u.createdAt);
  const infoRows=[
    ['E-posta',u.email||'—'],['Telefon',u.phone||'—'],['Doğum Tarihi',u.birthDate||'—'],['Üyelik Tarihi',joined],['Üyelik Durumu',u.disabled?'İptal Edildi':'Aktif'],['Telefon Doğrulama',u.phoneVerifiedAt?'Doğrulandı':'Doğrulanmadı'],['SMS İzni',u.smsMarketingConsent?'Açık':'Kapalı'],['E-posta İzni',u.emailMarketingConsent?'Açık':'Kapalı'],['Giriş Yöntemi',(u.authProviders||['password']).join(', ')],['Kayıtlı Adres',String(Number(u.addressCount||0))],['Toplam Sipariş',String(Number(u.orderCount||0))],['Tanımlı Kupon',String(Number(u.couponCount||0))]
  ];
  const infoHtml=infoRows.map(([label,value])=>`<div class="memberInfoLine"><span>${esc(label)}</span><b class="${label==='Üyelik Durumu'?(u.disabled?'memberStatusCancelled':'memberStatusActive'):''}">${esc(value)}</b></div>`).join('');
  return `<details class="memberAdminRow"><summary><span class="memberAdminPick"><span class="memberAdminIndex">${index+1}</span><label class="memberSelectBox" onclick="event.stopPropagation()"><input type="checkbox" ${checked} onchange="toggleMemberSelection('${attr(u.id)}',this.checked)"><span></span></label></span><span class="memberAdminName"><b>${esc([u.firstName,u.lastName].filter(Boolean).join(' ')||'İsimsiz Üye')}</b><small>${esc(u.email||'')}</small><small class="memberJoinedInline">Üye: ${esc(joined)}</small></span><strong>${esc(u.phone||'Telefon yok')}</strong><span class="memberAdminLocation"><b>${esc([u.province,u.district].filter(Boolean).join(' / ')||'Adres yok')}</b><small>${esc(u.address||'')}</small></span><span class="memberAdminOrders">${Number(u.orderCount||0)} sipariş<br><em>${Number(u.couponCount||0)} kupon tanımlı</em></span><span class="memberAdminArrow">⌄</span></summary><div class="memberAdminDetail memberAdminDetailCompact"><section class="memberInfoPanel"><div class="memberInfoPanelHead"><b>Üye Bilgileri</b>${u.disabled?'':`<button type="button" class="tinyInlineBtn dangerBtn" onclick="cancelMemberAccount('${attr(u.id)}','${attr([u.firstName,u.lastName].filter(Boolean).join(' ')||u.email||'Üye')}')">Üyeliği İptal Et</button>`}</div>${infoHtml}<div class="memberInfoLine"><span>Doğum Tarihi İşlemi</span><button type="button" class="tinyInlineBtn" onclick="editMemberBirthDate('${attr(u.id)}','${attr(u.birthDate||'')}')">Yönetimden değiştir</button></div></section><section class="memberSidePanel"><div class="memberSideCard"><h3>Tanımlı Kuponlar</h3>${adminCouponRowsHtml(u)}</div><div class="memberSideCard"><h3>Adresler</h3>${addresses}</div></section><section class="memberHistoryWide"><h3>Hesap Bilgisi Değişiklik Geçmişi</h3>${memberProfileHistoryHtml(u)}</section></div></details>`
