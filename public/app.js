    ...
    <div class="checkoutScrollBody addressScrollBody">
      <div class="addressCompact addressCompactV114">
        ...
        <div id="homeAddressFields" class="addressModeFields fieldWide addressModeFieldsV114" style="${branch?'display:none':''}">
          ...
          <div class="field fieldWide"><label><b>Adres devamı *</b><small class="fieldHelp">Bina no, kat, daire ve gerekiyorsa site/apartman adını yazın.</small></label><textarea class=formControl id=addr-fullAddress rows=2 placeholder="Örn: No:12 Kat:2 Daire:5" onpointerup="ensureAddressFieldFocus(this)" ${currentAccountUser&&checkoutState.manualAddressFromSaved?`data-address-suggest="fullAddress" onfocus="queueSavedAddressSuggestions(this,'fullAddress')" onclick="queueSavedAddressSuggestions(this,'fullAddress')" oninput="closeSavedAddressSuggestions()"`:''}>${escapeHtml(c.fullAddress||'')}</textarea></div>
        </div>
...
function ensureAddressFieldFocus(el){if(!el||document.activeElement===el)return;try{el.focus({preventScroll:true})}catch(_){try{el.focus()}catch(__){}}}
function addrInputInner(label,id,value='',type='text',placeholder=''){
  const suggest=currentAccountUser&&checkoutState.manualAddressFromSaved?` data-address-suggest="${escapeAttr(id)}" onfocus="queueSavedAddressSuggestions(this,'${escapeAttr(id)}')" onclick="queueSavedAddressSuggestions(this,'${escapeAttr(id)}')" oninput="closeSavedAddressSuggestions()"`:'';
  return `<label><b>${label}</b></label><input class=formControl id="addr-${id}" type="${type}" value="${escapeAttr(value||'')}" onpointerup="ensureAddressFieldFocus(this)" ${placeholder?`placeholder="${escapeAttr(placeholder)}"`:''}${suggest}>`;
}
...
function queueSavedAddressSuggestions(input,field){if(!input||!checkoutState.manualAddressFromSaved)return;clearTimeout(savedAddressSuggestionTimer);savedAddressSuggestionTimer=setTimeout(()=>{if(!document.body.contains(input)||document.activeElement!==input)return;openSavedAddressSuggestions(input,field)},20);setTimeout(()=>{const b=document.querySelector('.savedAddressSuggestionMenu');if(b&&activeSavedAddressSuggestionInput===input)positionSavedAddressSuggestions(b,input)},120)}
function openSavedAddressSuggestions(input,field){if(!input||!checkoutState.manualAddressFromSaved||!document.body.contains(input))return;const values=addressSuggestionValues(field);closeSavedAddressSuggestions();if(!values.length)return;const box=document.createElement('div');box.className='savedAddressSuggestionMenu';box.dataset.suggestionField=field;box.innerHTML=values.map(v=>`<button type="button" onpointerdown="event.preventDefault();event.stopPropagation()" data-value="${escapeAttr(v)}">${escapeHtml(v)}</button>`).join('');box.querySelectorAll('button').forEach(b=>b.onclick=e=>{e.preventDefault();input.value=b.dataset.value||'';closeSavedAddressSuggestions();try{input.focus({preventScroll:true})}catch(_){input.focus()}});document.body.appendChild(box);activeSavedAddressSuggestionInput=input;activeSavedAddressSuggestionField=field;positionSavedAddressSuggestions(box,input);requestAnimationFrame(()=>positionSavedAddressSuggestions(box,input));setTimeout(()=>{const live=document.querySelector('.savedAddressSuggestionMenu');if(live&&activeSavedAddressSuggestionInput===input)positionSavedAddressSuggestions(live,input)},160)}
...
function handleMarketingConsentToggle(el,channel){const was=channel==='sms'?!!currentAccountUser?.smsMarketingConsent:!!currentAccountUser?.emailMarketingConsent;if(!was||el.checked)return;el.checked=true;const text=channel==='sms'?'SMS pazarlama iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları SMS ile alamazsınız.':'E-posta pazarlama iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları e-posta ile alamazsınız.';pendingMarketingConsent={el,channel};profileConfirmOverlay('İletişim izni değişikliği',[text],()=>{if(pendingMarketingConsent?.el===el){el.checked=false;el.dataset.optoutConfirmed='1'}pendingMarketingConsent=null})}
...
async function commitAccountProfileSave(){const pending=pendingAccountProfileSave;if(!pending)return;pendingAccountProfileSave=null;const {body,btn}=pending;try{btn.disabled=true;const r=await apiJson('/api/account/profile',{method:'PATCH',body:JSON.stringify(body)});currentAccountUser=r.user;updateAccountHeader();toast(r.message||'✓ Hesap bilgileri güncellendi');showAccountProfile(false)}catch(e){alert(e.message)}finally{if(btn)btn.disabled=false}}
async function saveAccountProfile(btn){const required=['accFirst','accLast','accEmail','accPhone','accBirth'],missing=required.filter(id=>!String(document.getElementById(id)?.value||'').trim());if(missing.length)return showAuthErrors('Lütfen kırmızı işaretli zorunlu alanları doldurun.',missing);const body=accountProfileDraft();if(!body.birthDate)return showAuthErrors('Doğum tarihini gg.aa.yyyy şeklinde girin.',['accBirth']);const u=currentAccountUser||{},nameChanged=body.firstName!==String(u.firstName||'')||body.lastName!==String(u.lastName||''),phoneChanged=displayAccountPhone(body.phone)!==displayAccountPhone(u.phone||''),emailChanged=body.email.toLocaleLowerCase('tr-TR')!==String(u.email||'').trim().toLocaleLowerCase('tr-TR'),birthChanged=body.birthDate!==String(u.birthDate||''),smsChanged=body.smsMarketingConsent!==!!u.smsMarketingConsent,mailChanged=body.emailMarketingConsent!==!!u.emailMarketingConsent;if(!nameChanged&&!phoneChanged&&!emailChanged&&!birthChanged&&!smsChanged&&!mailChanged)return toast('Değişiklik bulunmuyor.');const rules=u.profileRules||{};if(nameChanged&&Number(rules.nameWaitMs||0)>0)return alert(`Ad / Soyad bilginizi tekrar değiştirebilmeniz için ${profileWaitText(rules.nameWaitMs)} kaldı.`);if(phoneChanged&&Number(rules.phoneWaitMs||0)>0)return alert(`Telefon numaranızı tekrar değiştirebilmeniz için ${profileWaitText(rules.phoneWaitMs)} kaldı.`);if(birthChanged&&rules.birthDateChangeAvailable===false)return alert('Doğum tarihinizi hesabınızdan daha önce değiştirdiniz. Bundan sonraki değişiklik yalnızca yönetim panelinden yapılabilir.');const warnings=[];if(nameChanged)warnings.push('Ad / Soyad bilgisi kaydedildikten sonra 30 gün boyunca tekrar değiştirilemez.');if(phoneChanged)warnings.push('Yeni telefon numarası doğrulanmadan mevcut telefon numaranız aktif kalır. Doğrulama tamamlandıktan sonra telefon değişikliği 7 gün boyunca tekrar yapılamaz.');if(emailChanged)warnings.push('Yeni e-posta adresi doğrulanmadan mevcut e-posta adresiniz aktif kalır.');if(birthChanged)warnings.push('Doğum tarihinizi hesabınızdan yalnızca bir kez değiştirebilirsiniz.');if(smsChanged&&u.smsMarketingConsent&&!body.smsMarketingConsent&&$('#accSms')?.dataset.optoutConfirmed!=='1')warnings.push('SMS pazarlama iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları SMS ile alamazsınız.');if(mailChanged&&u.emailMarketingConsent&&!body.emailMarketingConsent&&$('#accMail')?.dataset.optoutConfirmed!=='1')warnings.push('E-posta pazarlama iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları e-posta ile alamazsınız.');if(!warnings.length){pendingAccountProfileSave={body,btn};return commitAccountProfileSave()}pendingAccountProfileSave={body,btn};profileConfirmOverlay('Hesap bilgisi değişikliği',warnings,commitAccountProfileSave)}
