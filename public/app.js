...
function openNewCheckoutAddress(){checkoutState.manualAddressFromSaved=true;addressStep(true)}
...
function requiredOrderApprovalKeys(){return cart.some(x=>x.personalized)?['preInformation','distanceSales','personalization','personalizationNotice']:['preInformation','distanceSales']}
...
function approveOrderAndContinue(){
  ...
  shippingNotice(cart.some(x=>x.personalized));
}
...
function accountPopoverOutside(e){const p=document.querySelector('.accountEntryPopover');if(p&&!p.contains(e.target)&&!e.target.closest('#accountBtn'))p.remove()}
...
function authPasswordField(id,placeholder,autocomplete,watchMode=''){const watch=watchMode?`;updateAuthSubmitState('${watchMode}')`:'';return `<div class="authPasswordWrap"><input id="${id}" class="formControl" type="password" autocomplete="${autocomplete}" placeholder="${placeholder}" oninput="flashPasswordChar(this,event)${watch}"><span class="authLastCharFlash" aria-hidden="true"><span class="authLastCharText"></span><i class="authFlashCaret"></i></span><button type="button" class="authEye" onclick="toggleAuthPassword('${id}',this)" aria-label="Şifreyi göster">${authEyeSvg(false)}</button></div>`}
function toggleAuthPassword(id,btn){const x=document.getElementById(id);if(!x)return;const visible=x.type==='password';x.type=visible?'text':'password';btn.classList.toggle('active',visible);btn.innerHTML=authEyeSvg(visible);btn.setAttribute('aria-label',visible?'Şifreyi gizle':'Şifreyi göster');const wrap=x.parentElement,flash=wrap?.querySelector('.authLastCharFlash');if(flash){flash.classList.remove('show');clearTimeout(flash._hideTimer)}wrap?.classList.remove('maskFlashActive')}
function flashPasswordChar(input,e){const wrap=input?.parentElement,flash=wrap?.querySelector('.authLastCharFlash'),text=flash?.querySelector('.authLastCharText');if(!flash||!text||input.type!=='password'||!e?.data)return;const ch=String(e.data).slice(-1);if(!ch)return;const len=String(input.value||'').length;text.textContent='•'.repeat(Math.max(0,len-1))+ch;flash.classList.add('show');wrap.classList.add('maskFlashActive');clearTimeout(flash._hideTimer);flash._hideTimer=setTimeout(()=>{flash.classList.remove('show');wrap.classList.remove('maskFlashActive')},420)}
...
async function finishAccountLogin(user){
  currentAccountUser=user;setAccountHeaderHint(user);await loadAccountState();
  if(accountNeedsCompletion(currentAccountUser))return showSocialProfileCompletion();
  history.replaceState({...history.state,shazAccount:false},'','/');
  closeDrawer();closeAccountPopover();window.scrollTo({top:0,left:0,behavior:'auto'});
  centerToast(`Hoşgeldin ${currentAccountUser?.firstName||''}`.trim(),1500);
}
...
async function showAccountOrders(updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/siparisler');try{const r=await apiJson('/api/account/orders');const rows=(r.orders||[]).map(o=>`<div class="accountOrder"><div class="accountOrderTop"><b>Sipariş #${escapeHtml(o.dailyDisplayId||o.id)}</b><span>${money(o.total)}</span></div><small>${escapeHtml(o.createdAtTR||o.createdAt||'')} · ${escapeHtml(o.status||'')}</small><div class="accountRowActions"><button class="smallBtn" onclick="showAccountOrderDetail('${escapeAttr(o.id)}')">Detay</button></div></div>`).join('')||'<div class="accountEmptyState">Henüz hesabınıza bağlı sipariş bulunmuyor.</div>';openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Siparişlerim</h2></div>${rows}</div>`,'showAccountHome()'))}catch(e){alert(e.message)}}
...
async function showAccountOrderDetail(id,updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/siparisler/'+encodeURIComponent(id));try{const r=await apiJson('/api/account/orders/'+encodeURIComponent(id));const o=r.order,c=o.customer||{},tracking=o.trackingNo||o.shipping?.trackingNo||o.cargoTrackingNo||'',payment=o.payment?.label||o.payment?.method||o.payment||'';openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountOrders()">← Geri</button><h2>Sipariş #${escapeHtml(o.dailyDisplayId||o.id)}</h2></div><div class="wizardCard accountOrderMeta"><div><b>Durum</b><span>${escapeHtml(o.status||'')}</span></div><div><b>Sipariş Tarihi</b><span>${escapeHtml(o.createdAtTR||o.createdAt||'')}</span></div>${payment?`<div><b>Ödeme</b><span>${escapeHtml(String(payment))}</span></div>`:''}${tracking?`<div><b>Kargo Takip</b><span>${escapeHtml(String(tracking))}</span></div>`:''}<div><b>Telefon</b><span>${escapeHtml(c.phone||currentAccountUser?.phone||'')}</span></div><div class="accountOrderAddress"><b>Teslimat</b><span>${escapeHtml(customerAddressText(c)||'')}</span></div></div><div class="wizardCard"><h3>Ürünler</h3>${(o.items||[]).map(x=>`<div class="accountOrderItem"><div class="summaryLine"><span>${escapeHtml(x.product?.name||'Ürün')} × ${Number(x.qty||1)}</span><b>${money((x.product?.price||0)*(x.qty||1))}</b></div>${accountOrderPersonalizationHtml(x)}</div>`).join('')}</div><div class="wizardCard"><b>Toplam: ${money(o.total)}</b></div></div>`,'showAccountOrders()'))}catch(e){alert(e.message)}}
async function showAccountAddresses(updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/adresler');try{const response=await apiJson('/api/account/addresses');currentAccountAddresses=response.addresses||[];const rows=currentAccountAddresses.map(a=>{const title=escapeHtml(a.title||'Adres'),defaultTag=a.isDefault?'<span class="accountDefaultTag">Varsayılan Adres</span>':'';return `<div class="accountAddress ${a.isDefault?'is-default':''}" role="button" tabindex="0" onclick="if(!event.target.closest('button'))setDefaultAccountAddress('${escapeAttr(a.id)}')" onkeydown="if((event.key==='Enter'||event.key===' ')&&!event.target.closest('button')){event.preventDefault();setDefaultAccountAddress('${escapeAttr(a.id)}')}"><div class="accountAddressDefaultPick"><input type="radio" name="accountDefaultAddress" ${a.isDefault?'checked':''} aria-label="Varsayılan adres olarak seç" onclick="event.stopPropagation();setDefaultAccountAddress('${escapeAttr(a.id)}')"></div><div class="accountAddressBody"><div class="accountAddressTitle"><b>${title}</b>${defaultTag}</div><span class="accountAddressText">${escapeHtml([normalizeAddressDisplayPart(a.neighborhood,'neighborhood'),normalizeAddressDisplayPart(a.avenue,'avenue'),normalizeAddressDisplayPart(a.street,'street'),a.fullAddress,a.district,a.province].filter(Boolean).join(' '))}</span><span class="accountAddressPhone">${escapeHtml(displayAccountPhone(a.phone||''))}${a.extraPhone?` · 2. tel: ${escapeHtml(displayAccountPhone(a.extraPhone))}`:''}</span><div class="accountRowActions"><button class="smallBtn" onclick="event.stopPropagation();showAddressForm('${escapeAttr(a.id)}')">Düzenle</button><button class="smallBtn" onclick="event.stopPropagation();deleteAccountAddress('${escapeAttr(a.id)}')">Sil</button></div></div></div>`}).join('');openDrawer(accountStandardPage(`<div class="accountShell accountAddressesPage accountCenteredSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Adreslerim</h2></div>${rows||'<div class="accountEmptyState">Kayıtlı adresiniz yok.</div>'}<div class="accountAddressesToolbar"><button class="btn" onclick="showAddressForm('')">+ Yeni Adres Ekle</button></div></div>`,'showAccountHome()'))}catch(e){alert(e.message)}}
async function setDefaultAccountAddress(id){const a=currentAccountAddresses.find(x=>x.id===id);if(!a||a.isDefault)return;try{await apiJson('/api/account/addresses/'+encodeURIComponent(id),{method:'PATCH',body:JSON.stringify({isDefault:true})});await showAccountAddresses(false)}catch(e){alert(e.message)}}
function showAddressForm(id=''){const a=currentAccountAddresses.find(x=>x.id===id)||{};openDrawer(accountStandardPage(`<div class="accountShell"><div class="wizardHead"><button class="pill" onclick="showAccountAddresses()">← Geri</button><h2>${id?'Adresi Düzenle':'Yeni Adres'}</h2></div><div class="accountForm"><input id="aaTitle" class="formControl" value="${escapeAttr(a.title||'')}" placeholder="Adres Başlığı"><input id="aaFullName" class="formControl" value="${escapeAttr(a.fullName||[currentAccountUser.firstName,currentAccountUser.lastName].join(' '))}" placeholder="Ad Soyad"><input id="aaPhone" class="formControl" type="tel" inputmode="tel" value="${escapeAttr(displayAccountPhone(a.phone||currentAccountUser.phone||''))}" placeholder="Telefon"><input id="aaExtraPhone" class="formControl" type="tel" inputmode="tel" value="${escapeAttr(displayAccountPhone(a.extraPhone||''))}" placeholder="2. Telefon Numarası (isteğe bağlı)"><div class="authPair"><input id="aaProvince" class="formControl" value="${escapeAttr(a.province||'')}" placeholder="İl"><input id="aaDistrict" class="formControl" value="${escapeAttr(a.district||'')}" placeholder="İlçe"></div><input id="aaNeighborhood" class="formControl" value="${escapeAttr(stripAddressSuffix(a.neighborhood||'','neighborhood'))}" placeholder="Mahalle"><div class="authPair"><input id="aaAvenue" class="formControl" value="${escapeAttr(stripAddressSuffix(a.avenue||'','avenue'))}" placeholder="Cadde"><input id="aaStreet" class="formControl" value="${escapeAttr(stripAddressSuffix(a.street||'','street'))}" placeholder="Sokak"></div><textarea id="aaFullAddress" class="formControl" placeholder="Adres Devamı">${escapeHtml(a.fullAddress||'')}</textarea><button class="btn" onclick="saveAccountAddress('${escapeAttr(id)}',this)">Kaydet</button></div></div>`,'showAccountAddresses()'))}
...
function couponPopupKey(){return currentAccountUser?`shazCouponPopupShown:${currentAccountUser.id}`:''}
function couponPopupSignatureKey(){return currentAccountUser?`shazCouponPopupSignature:${currentAccountUser.id}`:''}
function couponNoticeSignature(){return activeAccountCoupons().map(c=>`${c.id||c.code}:${c.updatedAt||c.createdAt||''}:${c.status||''}`).join('|')}
...
function updateCouponNotificationBadges(){const show=couponNoticeDue();const head=$('#accountCouponBadge');if(head){head.textContent='1';head.classList.toggle('hidden',!show)}document.querySelectorAll('[data-coupon-notice-badge]').forEach(el=>el.classList.toggle('hidden',!show))}
function markCouponNotificationsViewed(){if(!currentAccountUser)return;try{localStorage.setItem(couponNoticeKey(),String(Date.now()));localStorage.setItem(couponNoticeSignatureKey(),couponNoticeSignature());localStorage.setItem(couponPopupKey(),String(Date.now()));localStorage.setItem(couponPopupSignatureKey(),couponNoticeSignature())}catch(_){}updateCouponNotificationBadges()}
...
function showAccountFavorites(updateRoute=true){if(!currentAccountUser)return showLogin(updateRoute);if(updateRoute)syncAccountRoute('/hesabim/favoriler');const ps=catalog.products.filter(p=>favorites.has(p.id));const content=ps.length?`<div class="memberFavoritesGrid">${ps.map(p=>{const img=mainProductImage(p);return `<div class="memberFavoriteCard"><button type="button" class="memberFavoritePhoto" onclick="openProductDetail('${p.id}','accountFavorites')">${img?`<img src="${escapeAttr(img)}" alt="${escapeAttr(p.name||'Ürün')}">`:'⌚'}</button><div class="memberFavoriteInfo"><b>${escapeHtml(p.name)}</b><span>${money(p.price)}</span><div class="accountRowActions"><button class="smallBtn" onclick="openProductDetail('${p.id}','accountFavorites')">Ürünü İncele</button><button class="smallBtn" onclick="removeFavoriteFromAccount('${p.id}')">Favoriden Kaldır</button></div></div></div>`}).join('')}</div>`:'<div class="accountEmptyState">Henüz favori ürününüz bulunmuyor.</div>';openDrawer(accountStandardPage(`<div class="accountShell memberFavoritesPage accountCenteredSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Favorilerim</h2></div>${content}</div>`,'showAccountHome()'))}
...
showFavorites=function(){if(!currentAccountUser)return showAuthChoice();const ps=catalog.products.filter(p=>favorites.has(p.id));openDrawer(`<div class="accountSubPage"><div class="wizardHead favoritesHead"><h2>Favorilerim <span class="favoritesTitleHeart">♥</span></h2><div class="favoritesHeadActions"><button class="pill" onclick="closeDrawer()">← Ana Sayfa</button></div></div>${ps.length?`<div class="favoritesList">${ps.map(p=>{const img=mainProductImage(p);return `<div class="favoriteCard"><button type="button" class="favoritePhoto" onclick="openProductDetail('${p.id}','favorites')">${img?`<img src="${escapeAttr(img)}" alt="${escapeAttr(p.name||'Ürün')}">`:'⌚'}</button><div class="favoriteContent"><div class="favoriteMeta"><b>${escapeHtml(p.name)}</b><span>${money(p.price)}</span></div><div class="favoriteActions"><button class="btn" onclick="openProductDetail('${p.id}','favorites')">Ürünü İncele</button><button class="pill" onclick="removeFavorite('${p.id}')">Favoriden Kaldır</button></div></div></div>`}).join('')}</div>`:'<div class="favoritesEmpty"><b>Henüz favoriniz yok.</b></div>'}</div>`)};
