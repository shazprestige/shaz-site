...
function adminMemberRows(){
  ...
  return users.map(u=>{
    const userAddresses=addresses.filter(a=>a.userId===u.id).map(a=>({...a}));
...
app.post('/api/auth/forgot-password',sameOriginGuard,async(req,res)=>{
  const identifier=String(req.body.identifier??req.body.email??'').trim(),email=normalizeEmail(identifier),phone=normalizeAccountPhone(identifier),users=readJson('users.json',[]),u=users.find(x=>(email.includes('@')&&x.email===email)||(phone&&x.phone===phone));
