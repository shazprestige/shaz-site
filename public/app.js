...
function announcementSignature(cfg={}){
  const raw=[cfg.eyebrow||'',cfg.title||'',cfg.text||'',cfg.imageUrl||'',cfg.buttonText||'',cfg.titleFontSize||30,cfg.textFontSize||14,cfg.buttonFontSize||14].join('|');
  let h=2166136261;
...
function renderSiteAnnouncement(force=false){
  ...
  if($('#siteAnnouncementText')) $('#siteAnnouncementText').textContent=cfg.text||'';const img=$('#siteAnnouncementImage');if(img){img.src=cfg.imageUrl||'';img.classList.toggle('hidden',!cfg.imageUrl)}
...
function closeSiteAnnouncement(){
  ...
  clearTimeout(window.__shazCouponAfterAnnouncement);window.__shazCouponAfterAnnouncement=setTimeout(()=>maybeShowCouponAnnouncement(),3000);
}
    ...
    <div class="checkoutScrollBody addressScrollBody">
      <div class="addressCompact addressCompactV114">
        ...
        <div class="addressPair phonePair fieldWide">
          <div class="field"><label><b>Telefon *</b><small class="fieldHelp phoneHelp">05xx xxx xx xx veya<br>5xx xxx xx xx</small></label>${currentAccountUser&&checkoutState.manualAddressFromSaved?`<div class="savedAddressSuggestWrap"><input class=formControl id=addr-phone type=tel inputmode=numeric autocomplete=off value="${escapeAttr(c.phone||'')}" placeholder="05xx xxx xx xx" data-address-suggest="phone" onfocus="queueSavedAddressSuggestions(this,'phone')" oninput="updateSavedAddressSuggestions(this,'phone')"></div>`:`<input class=formControl id=addr-phone type=tel inputmode=numeric autocomplete=tel value="${escapeAttr(c.phone||'')}" placeholder="05xx xxx xx xx">`}</div>
          <div class="field"><label><b>2. Telefon Numarası *</b><small class="fieldHelp">Size ulaşamamamız halinde kullanabileceğimiz ikinci numara zorunludur.</small></label>${currentAccountUser&&checkoutState.manualAddressFromSaved?`<div class="savedAddressSuggestWrap"><input class=formControl id=addr-extraPhone type=tel inputmode=numeric autocomplete=off value="${escapeAttr(c.extraPhone||'')}" placeholder="05xx xxx xx xx" data-address-suggest="extraPhone" onfocus="queueSavedAddressSuggestions(this,'extraPhone')" oninput="updateSavedAddressSuggestions(this,'extraPhone')"></div>`:`<input class=formControl id=addr-extraPhone type=tel inputmode=numeric autocomplete=tel value="${escapeAttr(c.extraPhone||'')}" placeholder="05xx xxx xx xx">`}</div>
        </div>
        ...
        <div id="homeAddressFields" class="addressModeFields fieldWide addressModeFieldsV114" style="${branch?'display:none':''}">
          ...
          <div class="field fieldWide"><label><b>Adres devamı *</b><small class="fieldHelp">Bina no, kat, daire ve gerekiyorsa site/apartman adını yazın.</small></label>${currentAccountUser&&checkoutState.manualAddressFromSaved?`<div class="savedAddressSuggestWrap"><textarea class=formControl id=addr-fullAddress rows=2 autocomplete=off placeholder="Örn: No:12 Kat:2 Daire:5" data-address-suggest="fullAddress" onfocus="queueSavedAddressSuggestions(this,'fullAddress')" oninput="updateSavedAddressSuggestions(this,'fullAddress')">${escapeHtml(c.fullAddress||'')}</textarea></div>`:`<textarea class=formControl id=addr-fullAddress rows=2 placeholder="Örn: No:12 Kat:2 Daire:5">${escapeHtml(c.fullAddress||'')}</textarea>`}</div>
        </div>
...
function addrInputInner(label,id,value='',type='text',placeholder=''){
  const canSuggest=!!(currentAccountUser&&checkoutState.manualAddressFromSaved);
  const suggest=canSuggest?` data-address-suggest="${escapeAttr(id)}" autocomplete="off" onfocus="queueSavedAddressSuggestions(this,'${escapeAttr(id)}')" oninput="updateSavedAddressSuggestions(this,'${escapeAttr(id)}')"`:'';
  const input=`<input class=formControl id="addr-${id}" type="${type}" value="${escapeAttr(value||'')}" ${placeholder?`placeholder="${escapeAttr(placeholder)}"`:''}${suggest}>`;
  return `<label><b>${label}</b></label>${canSuggest?`<div class="savedAddressSuggestWrap">${input}</div>`:input}`;
}
...
function queueSavedAddressSuggestions(input,field){if(!input||!checkoutState.manualAddressFromSaved)return;clearTimeout(savedAddressSuggestionTimer);savedAddressSuggestionTimer=setTimeout(()=>{if(document.body.contains(input)&&document.activeElement===input)openSavedAddressSuggestions(input,field)},0)}
function updateSavedAddressSuggestions(input,field){if(input&&document.activeElement===input)openSavedAddressSuggestions(input,field)}
function openSavedAddressSuggestions(input,field){if(!input||!checkoutState.manualAddressFromSaved||!document.body.contains(input))return;const values=matchingAddressSuggestionValues(field,input.value),wrap=input.closest('.savedAddressSuggestWrap');document.querySelector('.savedAddressSuggestionMenu')?.remove();if(!values.length||!wrap){activeSavedAddressSuggestionInput=null;activeSavedAddressSuggestionField='';return}const box=document.createElement('div');box.className='savedAddressSuggestionMenu';box.dataset.suggestionField=field;box.innerHTML=values.map(v=>`<button type="button" onpointerdown="event.preventDefault();event.stopPropagation()" data-value="${escapeAttr(v)}">${escapeHtml(v)}</button>`).join('');box.querySelectorAll('button').forEach(b=>b.onclick=e=>{e.preventDefault();input.value=b.dataset.value||'';closeSavedAddressSuggestions();ensureDirectInputFocus(input)});wrap.appendChild(box);activeSavedAddressSuggestionInput=input;activeSavedAddressSuggestionField=field}
if(!window._shazAddressSuggestionBound){window._shazAddressSuggestionBound=true;document.addEventListener('pointerdown',e=>{const b=document.querySelector('.savedAddressSuggestionMenu');if(b&&!b.contains(e.target)&&e.target!==activeSavedAddressSuggestionInput)closeSavedAddressSuggestions()},true)}
...
async function finalizeOrder(personalApproved,button){
  ...
  checkoutState.appliedCouponIds=couponPricing.coupons.map(c=>c.id);
  const order={
    requestId:checkoutState.requestId,deviceId:shazDeviceId(),
    items:cart,
...
function updateAccountHeader(){const t=$('#accountBtnText');if(!t)return;const name=currentAccountUser?(currentAccountUser.firstName||'Hesabım'):'Üyelik';t.textContent=name;t.classList.toggle('isLongAccountName',String(name).length>9);if(String(name).length>9)t.innerHTML=`<span>${escapeHtml(name)}</span><span aria-hidden="true">${escapeHtml(name)}</span>`}
...
async function openAccountEntry(){if(currentAccountUser&&couponNoticeCount())markCouponNotificationsViewed();const pop=document.querySelector('.accountEntryPopover');if(pop){if(pop.dataset.trigger==='account'){pop.remove();return}pop.remove();await replayAccountPopoverSwitch()}const drawer=$('#drawer');if(drawer&&!drawer.classList.contains('hidden')&&drawer.classList.contains('drawerAccountMode')){leaveAccountArea();return}if(!accountStateReady)await ensureAccountStateReady();currentAccountUser?showAccountHome():showAuthChoice('account')}
...
function positionAccountEntryPopover(el,anchor){if(!el||!anchor)return;const r=anchor.getBoundingClientRect(),w=el.offsetWidth||220,left=Math.max(12,Math.min(window.innerWidth-w-12,r.left+r.width/2-w/2));el.style.setProperty('--account-popover-left',left+'px');el.style.setProperty('--account-popover-top',(r.bottom+8)+'px');el.classList.add('accountEntryPopoverAnchored')}
function showAuthChoice(trigger='account'){
  ...
  document.body.appendChild(el);const anchor=document.getElementById(trigger==='favorites'?'favLink':'accountBtn');positionAccountEntryPopover(el,anchor);requestAnimationFrame(()=>positionAccountEntryPopover(el,anchor));setTimeout(()=>document.addEventListener('pointerdown',accountPopoverOutside,{once:true}),0)
}
...
function handleMarketingConsentClick(event,input,channel){
  if(input.checked)return true;
  event.preventDefault();
  const label=channel==='email'?'E-posta':'SMS';
  const el=document.createElement('div');el.className='accountLogoutConfirm marketingConsentConfirm';
  el.innerHTML=`<div class="accountLogoutConfirmCard"><b>${label} bildirimlerini kapatmak istediğinize emin misiniz?</b><div><button type="button" data-consent-yes>Evet, Kapat</button><button type="button" data-consent-no>Vazgeç</button></div></div>`;
  const stop=e=>e.stopPropagation();el.addEventListener('pointerdown',stop,true);document.body.appendChild(el);
  el.querySelector('[data-consent-yes]').onclick=e=>{e.preventDefault();e.stopPropagation();input.checked=false;el.remove()};
  el.querySelector('[data-consent-no]').onclick=e=>{e.preventDefault();e.stopPropagation();input.checked=true;el.remove()};
  return false
}
function accountOrderStatusInfo(status){const map={new:{label:'Siparişiniz alındı',step:0},prepared:{label:'Siparişiniz hazırlanıyor',step:1},shipped:{label:'Kargoya verildi',step:2},delivered:{label:'Sipariş teslim edildi',step:3}};return map[String(status||'').toLowerCase()]||{label:String(status||'Siparişiniz alındı'),step:0}}
function accountOrderTimeline(status){const current=accountOrderStatusInfo(status).step,steps=['Sipariş alındı','Hazırlanıyor','Kargoya verildi','Teslim edildi'];return `<div class="accountOrderTimeline">${steps.map((label,i)=>`${i?'<span class="accountOrderTrack"></span>':''}<span class="accountOrderStep ${i<=current?'is-done':''} ${i===current?'is-current':''}"><i></i><small>${label}</small></span>`).join('')}</div>`}
function accountPaymentLabel(payment){const raw=typeof payment==='string'?payment:String(payment?.method||payment?.type||'');const map={cod:'Kapıda Ödeme',cash:'Kapıda Ödeme',online:'Online Ödeme',card:'Kart ile Ödeme',bank:'Havale / EFT'};return map[raw.toLowerCase()]||raw}
function accountOrderItemImage(x){const p=x?.product||{};return p.image||(Array.isArray(p.images)?p.images.find(Boolean):'')||x?.image||''}
function accountOrderPersonalizationHtml(x){const writes=x?.writes||x?.setCustomization?.writes||[],photos=x?.photoCustomizations||x?.setCustomization?.photoCustomizations||[],note=String(x?.productNote||'').trim(),rows=[];writes.forEach(w=>rows.push(`<div class="accountPersonalLine"><b>${escapeHtml(w.item||'Yazı')}</b><span>${escapeHtml(w.text||'')}${w.position?` · ${escapeHtml(w.position)}`:''}</span></div>`));photos.forEach(ph=>rows.push(`<div class="accountPersonalLine"><b>${escapeHtml(ph.item||'Fotoğraf')}</b><span>Fotoğraf işleme${ph.caption?` · ${escapeHtml(ph.caption)}`:''}</span></div>`));if(note)rows.push(`<div class="accountPersonalLine"><b>Ürün notu</b><span>${escapeHtml(note)}</span></div>`);return rows.length?`<div class="accountPersonalBox"><strong>Kişiselleştirme</strong>${rows.join('')}</div>`:''}
function accountOrderListHtml(){if(!currentAccountOrders.length)return '<div class="accountEmptyState">Henüz siparişiniz bulunmuyor.</div>';return `<div class="accountOrderList">${[...currentAccountOrders].sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0)).map(o=>{const info=accountOrderStatusInfo(o.status),items=Array.isArray(o.items)?o.items:[],thumbs=items.slice(0,3).map(x=>{const img=accountOrderItemImage(x);return img?`<span><img src="${escapeAttr(img)}" alt=""></span>`:''}).join(''),more=Math.max(0,items.length-3);return `<button type="button" class="wizardCard accountOrderListCard" onclick="showAccountOrderDetail('${escapeAttr(o.id)}')"><div class="accountOrderListHead"><b>Sipariş #${escapeHtml(o.dailyDisplayId||o.id)}</b><span>${escapeHtml(o.createdAtTR||o.createdAt||'')}</span></div>${thumbs?`<div class="accountOrderThumbs">${thumbs}${more?`<b>+${more}</b>`:''}</div>`:''}<div class="accountOrderListStatus">${escapeHtml(info.label)}</div><strong>${money(o.total||0)}</strong></button>`}).join('')}</div>`}
function renderAccountOrdersPage(){openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Siparişlerim</h2></div>${accountOrderListHtml()}</div>`,'showAccountHome()'))}
async function showAccountOrders(updateRoute=true){try{document.activeElement?.blur()}catch(_){}if(updateRoute)syncAccountRoute('/hesabim/siparisler');renderAccountOrdersPage();try{const r=await apiJson('/api/account/orders');currentAccountOrders=Array.isArray(r.orders)?r.orders:[];if(location.pathname==='/hesabim/siparisler')renderAccountOrdersPage()}catch(e){console.warn('Siparişler yenilenemedi:',e.message)}}
...
function couponNoticeDue(){if(!currentAccountUser||!activeAccountCoupons().length)return false;try{const sig=couponNoticeSignature(),seen=localStorage.getItem(couponNoticeSignatureKey())||'';return sig!==seen}catch(_){return true}}
...
async function refreshAccountCoupons(){if(!currentAccountUser)return;try{const r=await apiJson('/api/account/coupons');currentAccountCoupons=Array.isArray(r.coupons)?r.coupons:[];updateCouponNotificationBadges()}catch(_){}}
...
function couponNoticeTerms(c){const rows=[];if(c.discountType==='percent'&&Number(c.maxDiscountAmount||0)>0)rows.push(`Maksimum indirim: ${money(c.maxDiscountAmount)}`);rows.push(c.stackable?'Diğer uygun kuponlarla birlikte kullanılabilir':'Diğer kuponlarla birlikte kullanılamaz');return rows}
function maybeShowCouponAnnouncement(){
  ...
  el.innerHTML=`<div class="couponNoticeCard"><button type="button" class="couponNoticeClose" onclick="this.closest('.couponNoticeOverlay').remove()">×</button><h3>${escapeHtml(heading)}</h3><div class="couponNoticeList">${rows.map(c=>{const ex=couponExpiryInfo(c),min=Number(c.minCartAmount||0),terms=couponNoticeTerms(c);return `<div class="couponNoticeItem">${rows.length>1&&String(c.headline||'').trim()?`<h4 class="couponNoticeItemHeadline">${escapeHtml(c.headline)}</h4>`:''}<strong class="couponNoticeDiscount">${escapeHtml(couponDiscountLabel(c))}</strong>${min>0?`<p class="couponNoticeMinimum">Minimum sepet tutarı: ${money(min)}</p>`:''}${String(c.title||'').trim()?`<p class="couponNoticeDescription">${escapeHtml(c.title)}</p>`:''}${String(c.code||'').trim()?`<div class="couponNoticeCodeSecondary">Kupon kodu: <b>${escapeHtml(c.code)}</b></div>`:''}${terms.length?`<div class="couponNoticeRules">${terms.map(x=>`<small>${escapeHtml(x)}</small>`).join('')}</div>`:''}<small class="couponNoticeExpiry ${ex.urgent?'couponExpiryUrgent':''}">${escapeHtml(ex.text)}</small></div>`}).join('')}</div><button type="button" class="btn" onclick="this.closest('.couponNoticeOverlay').remove();showAccountCoupons()">Kuponlarımı Gör</button></div>`;
  document.body.appendChild(el)
}
...
function confirmLogoutAccount(){if(document.querySelector('.accountLogoutConfirm'))return;const el=document.createElement('div');el.className='accountLogoutConfirm';el.innerHTML=`<div class="accountLogoutConfirmCard"><b>Çıkış yapılsın mı?</b><div><button type="button" data-logout-yes>Evet</button><button type="button" data-logout-no>Hayır</button></div></div>`;el.addEventListener('click',e=>{const yes=e.target.closest('[data-logout-yes]'),no=e.target.closest('[data-logout-no]');if(!yes&&!no)return;e.preventDefault();e.stopPropagation();el.remove();if(yes)requestAnimationFrame(()=>logoutAccount());else requestAnimationFrame(()=>{try{document.activeElement?.blur()}catch(_){}})});document.body.appendChild(el)}
...
async function trackShazVisit(){const id=shazDeviceId();if(!id)return;await fetch('/api/activity/visit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({deviceId:id,pwa:isStandalonePwa(),permission:('Notification'in window?Notification.permission:'unsupported')})}).catch(()=>{})}
