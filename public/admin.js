...
function memberRowsHtml(){const filtered=memberFiltered();return filtered.map((u,index)=>{
  ...
  return `<details class="memberAdminRow"><summary><span class="memberAdminPick"><span class="memberAdminIndex">${index+1}</span><label class="memberSelectBox" onclick="event.stopPropagation()"><input type="checkbox" ${checked} onchange="toggleMemberSelection('${attr(u.id)}',this.checked)"><span></span></label></span><span class="memberAdminName"><b>${esc([u.firstName,u.lastName].filter(Boolean).join(' ')||'İsimsiz Üye')}</b><small>${esc(u.email||'')}</small></span><strong>${esc(u.phone||'Telefon yok')}</strong><span class="memberAdminLocation"><b>${esc([u.province,u.district].filter(Boolean).join(' / ')||'Adres yok')}</b><small>${esc(u.address||'')}</small></span><span class="memberAdminOrders">${Number(u.orderCount||0)} sipariş<br><em>${Number(u.couponCount||0)} kupon tanımlı</em></span><span class="memberAdminArrow">⌄</span></summary><div class="memberAdminDetail"><div><span>E-posta</span><b>${esc(u.email||'—')}</b></div><div><span>Telefon</span><b>${esc(u.phone||'—')}</b></div><div><span>Doğum Tarihi</span><b>${esc(u.birthDate||'—')}</b><button type="button" class="tinyInlineBtn" onclick="editMemberBirthDate('${attr(u.id)}','${attr(u.birthDate||'')}')">Yönetimden değiştir</button></div><div><span>Üyelik Tarihi</span><b>${esc(formatAdminDate(u.createdAt))}</b></div><div><span>Üyelik Durumu</span><b class="${u.disabled?'memberStatusCancelled':'memberStatusActive'}">${u.disabled?'İptal Edildi':'Aktif'}</b>${u.disabled?'':`<button type="button" class="tinyInlineBtn dangerBtn" onclick="cancelMemberAccount('${attr(u.id)}','${attr([u.firstName,u.lastName].filter(Boolean).join(' ')||u.email||'Üye')}')">Üyeliği İptal Et</button>`}</div><div><span>Telefon Doğrulama</span><b>${u.phoneVerifiedAt?'Doğrulandı':'Doğrulanmadı'}</b></div><div><span>SMS İzni</span><b>${u.smsMarketingConsent?'Açık':'Kapalı'}</b></div><div><span>E-posta İzni</span><b>${u.emailMarketingConsent?'Açık':'Kapalı'}</b></div><div><span>Giriş Yöntemi</span><b>${esc((u.authProviders||['password']).join(', '))}</b></div><div><span>Kayıtlı Adres</span><b>${Number(u.addressCount||0)}</b></div><div><span>Toplam Sipariş</span><b>${Number(u.orderCount||0)}</b></div><div><span>Tanımlı Kupon</span><b>${Number(u.couponCount||0)}</b></div><div class="memberAllAddresses"><span>Tüm Adres Bilgileri</span>${addresses}</div><div class="memberAllAddresses"><span>Tanımlı Kuponlar</span>${adminCouponRowsHtml(u)}</div><div class="memberAllAddresses"><span>Hesap Bilgisi Değişiklik Geçmişi</span>${memberProfileHistoryHtml(u)}</div></div></details>`
