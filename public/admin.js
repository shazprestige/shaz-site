...
let memberFilterState={search:'',province:'',district:'',sms:'',emailMarketing:'',provider:'',coupon:'',dateFrom:'',dateTo:'',sort:'newest'};
...
async function load(){
  ...
  settings.siteAnnouncement=settings.siteAnnouncement||{enabled:false,eyebrow:'DUYURU',title:'',text:'',imageUrl:'',buttonText:'Kapat',fontScale:100};
  ...
  show(['site','catalog','custom','discounts','upsells','orders','builderAccess','soldout','legal','members','customers','notifications'].includes(savedTab)?savedTab:'site');
...
function show(tab){
  ...
  const isOrders=tab==='orders'||tab==='members'||tab==='customers'||tab==='notifications';
  ...
  if(tab==='members')return renderMembers();if(tab==='customers')return renderAllCustomers();if(tab==='notifications')return renderNotificationSettings();
}

function formatAdminDate(v){if(!v)return '—';try{return new Date(v).toLocaleString('tr-TR')}catch{return String(v)}}
function memberAddressLine(a){return [a?.neighborhood,a?.avenue,a?.street,a?.fullAddress].filter(Boolean).join(' ')}
function memberFiltered(){
  const search=memberFilterState.search.trim().toLocaleLowerCase('tr-TR'),province=memberFilterState.province,district=memberFilterState.district,sms=memberFilterState.sms,emailMarketing=memberFilterState.emailMarketing,provider=memberFilterState.provider,coupon=memberFilterState.coupon;
  const fromTs=memberFilterState.dateFrom?new Date(memberFilterState.dateFrom+'T00:00:00').getTime():0,toTs=memberFilterState.dateTo?new Date(memberFilterState.dateTo+'T23:59:59.999').getTime():0;
  const out=(members||[]).filter(u=>{const hay=[u.firstName,u.lastName,u.email,u.phone,u.province,u.district,u.address].join(' ').toLocaleLowerCase('tr-TR'),providers=(u.authProviders||[]).map(x=>String(x).toLowerCase()),createdTs=new Date(u.createdAt||0).getTime();return (!search||hay.includes(search))&&(!province||u.province===province)&&(!district||u.district===district)&&(!sms||String(!!u.smsMarketingConsent)===sms)&&(!emailMarketing||String(!!u.emailMarketingConsent)===emailMarketing)&&(!provider||providers.includes(provider))&&(!coupon||(coupon==='yes'?Number(u.couponCount||0)>0:Number(u.couponCount||0)===0))&&(!fromTs||createdTs>=fromTs)&&(!toTs||createdTs<=toTs)});
  out.sort((a,b)=>{if(memberFilterState.sort==='lastSeenDesc'||memberFilterState.sort==='lastSeenAsc'){const ta=Date.parse(a.lastSeenAt||''),tb=Date.parse(b.lastSeenAt||''),va=Number.isFinite(ta),vb=Number.isFinite(tb);if(!va&&!vb)return 0;if(!va)return 1;if(!vb)return -1;return memberFilterState.sort==='lastSeenAsc'?ta-tb:tb-ta}return memberFilterState.sort==='name'?`${a.firstName} ${a.lastName}`.localeCompare(`${b.firstName} ${b.lastName}`,'tr'):memberFilterState.sort==='orders'?Number(b.orderCount||0)-Number(a.orderCount||0):memberFilterState.sort==='coupons'?Number(b.couponCount||0)-Number(a.couponCount||0):memberFilterState.sort==='province'?`${a.province} ${a.district}`.localeCompare(`${b.province} ${b.district}`,'tr'):new Date(b.createdAt||0)-new Date(a.createdAt||0)});
  return out;
}
function adminCouponTypeLabel(c){return c.discountType==='fixed'?`${Number(c.value||0).toLocaleString('tr-TR')} TL İNDİRİM`:`%${Number(c.value||0).toLocaleString('tr-TR')} İNDİRİM`}
function adminCouponRowsHtml(u){const rows=u.coupons||[];if(!rows.length)return '<div class="memberEmptyInline">Tanımlı kupon yok.</div>';return `<div class="memberCouponInventory">${rows.map(c=>`<div class="memberCouponInventoryRow"><div><b>${esc(c.code||'')}</b><strong>${esc(adminCouponTypeLabel(c))}</strong><small>${c.expiresAt?`Son kullanım: ${esc(String(c.expiresAt).slice(0,10))}`:'Son kullanım tarihi yok'}${Number(c.minCartAmount||0)>0?` · Min. ${Number(c.minCartAmount).toLocaleString('tr-TR')} TL`:''}</small></div><div><button class="smallBtn" type="button" onclick="event.stopPropagation();openEditMemberCouponModal('${attr(u.id)}','${attr(c.id)}')">Düzenle</button><button class="smallBtn dangerBtn" type="button" onclick="event.stopPropagation();deleteMemberCoupon('${attr(c.id)}')">Sil</button></div></div>`).join('')}</div>`}
function memberProfileHistoryHtml(u){const rows=[...(u.profileChangeHistory||[])].reverse();if(!rows.length)return '<div class="memberEmptyInline">Hesap bilgisi değişiklik geçmişi yok.</div>';return `<div class="memberProfileHistory">${rows.map(h=>`<div><time>${esc(formatAdminDate(h.at))}</time><b>${esc(h.field||'Alan')}</b><span>${esc(h.oldValue||'—')} → ${esc(h.newValue||'—')}</span>${h.source==='admin'?'<small>Yönetim panelinden</small>':''}</div>`).join('')}</div>`}
function memberRowsHtml(){const filtered=memberFiltered(),allByAge=[...(members||[])].sort((a,b)=>new Date(a.createdAt||0)-new Date(b.createdAt||0)),seq=new Map(allByAge.map((u,i)=>[u.id,i+1]));return filtered.map(u=>{
  const addresses=(u.addresses||[]).map(a=>`<div class="memberAddressBlock"><b>${esc(a.title||'Adres')}${a.isDefault?' · Varsayılan':''}</b><span>${esc(a.fullName||'')}</span><span>1. Tel: ${esc(a.phone||'—')}</span><span>2. Tel: ${esc(a.extraPhone||'—')}</span><span>${esc([a.province,a.district,a.neighborhood,a.avenue,a.street,a.fullAddress].filter(Boolean).join(' · '))}</span></div>`).join('')||'<div class="memberAddressBlock">Kayıtlı adres yok.</div>';
  const checked=selectedMemberIds.has(u.id)?'checked':'',joined=formatAdminDate(u.createdAt),name=[u.firstName,u.lastName].filter(Boolean).join(' ')||'İsimsiz Üye',lastSeen=formatAdminDate(u.lastSeenAt);
  const edit=(label,field,val)=>`<div class="memberInfoLine"><span>${esc(label)}</span><b>${esc(val||'—')}</b><button type="button" class="tinyInlineBtn" onclick="editMemberField('${attr(u.id)}','${field}','${attr(val||'')}','${attr(label)}')">Düzenle</button></div>`;
  return `<details class="memberAdminRow"><summary><span class="memberAdminPick"><span class="memberAdminIndex">${seq.get(u.id)||''}</span><label class="memberSelectBox" onclick="event.stopPropagation()"><input type="checkbox" ${checked} onchange="toggleMemberSelection('${attr(u.id)}',this.checked)"><span></span></label></span><span class="memberAdminName"><b>${esc(name)}</b><small>${esc(u.email||'')}</small></span><span class="memberPhones"><b>${esc(u.phone||'Telefon yok')}</b><small>${esc(u.extraPhone||'')}</small></span><span class="memberAdminLocation"><b>${esc([u.province,u.district].filter(Boolean).join(' / ')||'Adres yok')}</b><small>${esc(u.address||'')}</small></span><span class="memberLastSeen"><b>${esc(lastSeen)}</b><button type="button" class="tinyInlineBtn" onclick="event.stopPropagation();showMemberVisits('${attr(u.id)}')">Tüm Girişler (${Number(u.visitCount||0)})</button></span><span class="memberAdminOrders">${Number(u.orderCount||0)} sipariş<br><em>${Number(u.couponCount||0)} kupon tanımlı</em></span><span class="memberStatusSummary ${u.disabled?'memberStatusCancelled':'memberStatusActive'}">${u.disabled?'İptal':'Aktif'}</span></summary><div class="memberAdminDetail memberAdminDetailCompact"><section class="memberInfoPanel"><div class="memberInfoPanelHead"><b>Üye Bilgileri</b><div class="memberAccountActions">${u.disabled?`<button class="tinyInlineBtn" onclick="reactivateMemberAccount('${attr(u.id)}')">Üyeliği Aç</button>`:`<button class="tinyInlineBtn dangerBtn" onclick="cancelMemberAccount('${attr(u.id)}','${attr(name)}')">Üyeliği İptal Et</button>`}<button class="tinyInlineBtn dangerBtn" onclick="deleteMemberAccount('${attr(u.id)}','${attr(name)}')">İçeriği Sil</button></div></div>${edit('Ad','firstName',u.firstName)}${edit('Soyad','lastName',u.lastName)}${edit('E-posta','email',u.email)}${edit('Telefon','phone',u.phone)}${edit('Doğum Tarihi','birthDate',u.birthDate)}<div class="memberInfoLine"><span>Üyelik Tarihi</span><b>${esc(joined)}</b></div><div class="memberInfoLine"><span>Üyelik Durumu</span><b class="${u.disabled?'memberStatusCancelled':'memberStatusActive'}">${u.disabled?'İptal Edildi':'Aktif'}</b></div><div class="memberInfoLine"><span>SMS İzni</span><b>${u.smsMarketingConsent?'Açık':'Kapalı'}</b></div><div class="memberInfoLine"><span>E-posta İzni</span><b>${u.emailMarketingConsent?'Açık':'Kapalı'}</b></div></section><section class="memberSidePanel"><div class="memberSideCard"><h3>Tanımlı Kuponlar</h3>${adminCouponRowsHtml(u)}</div><div class="memberSideCard"><h3>Adresler</h3>${addresses}</div></section><section class="memberHistoryWide"><h3>Hesap Bilgisi Değişiklik Geçmişi</h3>${memberProfileHistoryHtml(u)}</section></div></details>`
}).join('')||'<div class="panel">Bu filtrelere uygun üye bulunamadı.</div>'}
function showMemberVisits(id){const u=(members||[]).find(x=>x.id===id);alert((u?.visits||[]).map(x=>formatAdminDate(x)).join('\n')||'Giriş kaydı yok.')}
function updateMemberRows(){const rows=$('#memberRows'),count=$('#memberCount'),selected=$('#memberSelectedCount');if(rows)rows.innerHTML=memberRowsHtml();if(count)count.textContent=memberFiltered().length;if(selected)selected.textContent=selectedMemberIds.size;const btn=$('#assignCouponBtn');if(btn)btn.disabled=!selectedMemberIds.size}
function updateMemberFilter(key,value){memberFilterState[key]=value;if(key==='province'){const valid=new Set((members||[]).filter(u=>!value||u.province===value).map(u=>u.district).filter(Boolean));if(memberFilterState.district&&!valid.has(memberFilterState.district))memberFilterState.district='';renderMembers(false);return}updateMemberRows()}
function toggleMemberSelection(id,on){on?selectedMemberIds.add(id):selectedMemberIds.delete(id);updateMemberRows()}
function toggleAllFilteredMembers(on){memberFiltered().forEach(u=>on?selectedMemberIds.add(u.id):selectedMemberIds.delete(u.id));updateMemberRows()}
function exportMembersExcel(){const q=new URLSearchParams({search:memberFilterState.search,province:memberFilterState.province,district:memberFilterState.district,sms:memberFilterState.sms,emailMarketing:memberFilterState.emailMarketing,provider:memberFilterState.provider,dateFrom:memberFilterState.dateFrom,dateTo:memberFilterState.dateTo,sort:memberFilterState.sort});window.location.href='/api/admin/users/export.xlsx?'+q.toString()}
function adminTodayDateKey(){const d=new Date(),off=d.getTimezoneOffset()*60000;return new Date(d-off).toISOString().slice(0,10)}
function memberCouponDialogHtml(c=null,userId=''){const edit=!!c;return `<div class="memberCouponDialog"><div class="memberCouponHead"><div><h2>${edit?'Kuponu Düzenle':'Kupon Tanımla'}</h2><p>${edit?`<b>${esc(c.code||'')}</b> kuponunun yalnız bu müşteriye ait ayarlarını düzenliyorsunuz.`:`<b>${selectedMemberIds.size}</b> seçili üyeye kupon tanımlanacak. Her atama bağımsız kupon oluşturur.`}</p></div><button class="smallBtn" onclick="closeMemberCouponModal()">Kapat</button></div><div class="memberCouponGrid"><div class="field memberCouponWide"><label><b>Müşteriye Gösterilecek Başlık</b></label><input id="memberCouponHeadline" class="formControl" value="${attr(c?.headline||'')}" placeholder="Örn: Hafta Sonu Fırsatı"></div><div class="field"><label><b>Kupon Kodu *</b></label><input id="memberCouponCode" class="formControl" maxlength="32" value="${attr(c?.code||'')}" placeholder="Örn: SHAZ100"></div><div class="field"><label><b>İndirim Türü *</b></label><select id="memberCouponType" class="formControl" onchange="syncMemberCouponFields()"><option value="percent" ${c?.discountType!=='fixed'?'selected':''}>Yüzde (%)</option><option value="fixed" ${c?.discountType==='fixed'?'selected':''}>Sabit TL</option></select></div><div class="field"><label><b>İndirim Değeri *</b></label><input id="memberCouponValue" class="formControl" type="number" min="1" step="1" value="${attr(c?.value??'')}" placeholder="Örn: 10"></div><div class="field"><label><b>Son Kullanma Tarihi</b></label><input id="memberCouponExpiry" class="formControl" type="date" min="${adminTodayDateKey()}" value="${attr(String(c?.expiresAt||'').slice(0,10))}"><small>Seçilen gün Türkiye saatiyle 23:59:59 sonuna kadar geçerlidir.</small></div><div class="field"><label><b>Minimum Sepet Tutarı</b></label><input id="memberCouponMinCart" class="formControl" type="number" min="0" step="1" value="${attr(c?.minCartAmount??'')}" placeholder="Örn: 1000"></div><div class="field"><label><b>Diğer Kuponlarla Birlikte Kullanılabilir</b></label><select id="memberCouponStackable" class="formControl"><option value="no" ${!c?.stackable?'selected':''}>Hayır</option><option value="yes" ${c?.stackable?'selected':''}>Evet</option></select><small id="memberCouponStackableHelp"></small></div><div class="field memberCouponMaxField" id="memberCouponMaxField"><label><b>Maksimum İndirim Tutarı</b></label><input id="memberCouponMaxDiscount" class="formControl" type="number" min="0" step="1" value="${attr(c?.maxDiscountAmount??'')}" placeholder="Örn: 300"></div><div class="field memberCouponWide"><label><b>Açıklama</b></label><input id="memberCouponTitle" class="formControl" value="${attr(c?.title||'')}" placeholder="Örn: Üyeye özel kupon"></div></div><button class="btn" onclick="${edit?`saveEditedMemberCoupon('${attr(c.id)}',this)`:'assignCouponToMembers(this)'}">${edit?'Kuponu Kaydet':'Seçili Üyelere Kupon Tanımla'}</button></div>`}
function openMemberCouponModal(){if(!selectedMemberIds.size)return alert('Önce kupon tanımlamak istediğin üyeleri seç.');document.querySelector('.memberCouponOverlay')?.remove();const el=document.createElement('div');el.className='memberCouponOverlay';el.innerHTML=memberCouponDialogHtml();document.body.appendChild(el);syncMemberCouponFields();el.addEventListener('click',e=>{if(e.target===el)el.remove()})}
function openEditMemberCouponModal(userId,couponId){const u=(members||[]).find(x=>String(x.id)===String(userId)),c=u?.coupons?.find(x=>String(x.id)===String(couponId));if(!c)return alert('Kupon bulunamadı.');document.querySelector('.memberCouponOverlay')?.remove();const el=document.createElement('div');el.className='memberCouponOverlay';el.innerHTML=memberCouponDialogHtml(c,userId);document.body.appendChild(el);syncMemberCouponFields();el.addEventListener('click',e=>{if(e.target===el)el.remove()})}
function syncMemberCouponFields(){const percent=$('#memberCouponType')?.value==='percent',field=$('#memberCouponMaxField'),stack=$('#memberCouponStackable'),help=$('#memberCouponStackableHelp');if(field)field.style.display=percent?'grid':'none';if(stack){if(percent){stack.value='no';stack.disabled=true}else stack.disabled=false}if(help)help.textContent=percent?'Yüzdelik kupon başka kuponlarla birleştirilemez.':''}
function closeMemberCouponModal(){document.querySelector('.memberCouponOverlay')?.remove()}
function memberCouponFormBody(extra={}){return {...extra,code:$('#memberCouponCode')?.value,discountType:$('#memberCouponType')?.value,value:Number($('#memberCouponValue')?.value||0),expiresAt:$('#memberCouponExpiry')?.value,title:$('#memberCouponTitle')?.value,headline:$('#memberCouponHeadline')?.value,minCartAmount:$('#memberCouponMinCart')?.value,stackable:$('#memberCouponStackable')?.value,maxDiscountAmount:$('#memberCouponMaxDiscount')?.value,validityDays:Number($('#memberCouponValidityDays')?.value||0)}}
async function assignCouponToMembers(btn){const body=memberCouponFormBody({userIds:[...selectedMemberIds]});if(body.expiresAt&&body.expiresAt<adminTodayDateKey())return alert('Geçmiş bir tarihe kupon tanımlanamaz.');if(!confirm(`${selectedMemberIds.size} üyeye ${String(body.code||'').toUpperCase()} kuponu tanımlansın mı?`))return;try{btn.disabled=true;const r=await fetch('/api/admin/users/coupons',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Kupon tanımlanamadı.');alert(`${r.assigned} üyeye ${r.code} kuponu tanımlandı.`);selectedMemberIds.clear();closeMemberCouponModal();await renderMembers(true)}catch(e){alert(e.message)}finally{btn.disabled=false}}
async function saveEditedMemberCoupon(id,btn){const body=memberCouponFormBody();if(body.expiresAt&&body.expiresAt<adminTodayDateKey())return alert('Geçmiş bir tarihe kupon ayarlanamaz.');if(!confirm('Bu kuponun ayarları değiştirilsin mi?'))return;try{btn.disabled=true;const r=await fetch('/api/admin/coupons/'+encodeURIComponent(id),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Kupon güncellenemedi.');closeMemberCouponModal();await renderMembers(true)}catch(e){alert(e.message)}finally{btn.disabled=false}}
async function deleteMemberCoupon(id){if(!confirm('Bu kupon silinsin mi? Bu işlem müşterinin hesabından kuponu kaldırır.'))return;try{const r=await fetch('/api/admin/coupons/'+encodeURIComponent(id),{method:'DELETE'}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Kupon silinemedi.');await renderMembers(true)}catch(e){alert(e.message)}}
async function editMemberBirthDate(userId,current){const value=prompt('Yeni doğum tarihini YYYY-AA-GG formatında girin.',current||'');if(value===null)return;if(!/^\d{4}-\d{2}-\d{2}$/.test(value))return alert('Tarih YYYY-AA-GG formatında olmalı.');if(!confirm(`Doğum tarihi ${value} olarak değiştirilsin mi?`))return;try{const r=await fetch('/api/admin/users/'+encodeURIComponent(userId)+'/profile',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({birthDate:value})}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Doğum tarihi değiştirilemedi.');await renderMembers(true)}catch(e){alert(e.message)}}

function autoCouponTemplateRowsHtml(){if(!newMemberCouponTemplates.length)return '<div class="memberEmptyInline">Henüz yeni üyelere otomatik atanacak kupon tanımlanmadı.</div>';return `<div class="autoCouponTemplateList">${newMemberCouponTemplates.map(c=>`<div class="autoCouponTemplateRow"><div><b>${esc(c.headline||'Yeni üye kuponu')}</b><strong>${esc(c.code||'')} · ${esc(adminCouponTypeLabel(c))}</strong><small>Üyelikten sonra ${Number(c.validityDays||7)} gün geçerli${Number(c.minCartAmount||0)>0?` · Min. ${Number(c.minCartAmount).toLocaleString('tr-TR')} TL`:''}</small></div><div><button class="smallBtn" type="button" onclick="openNewMemberAutoCouponModal('${attr(c.id)}')">Düzenle</button><button class="smallBtn dangerBtn" type="button" onclick="deleteNewMemberAutoCoupon('${attr(c.id)}')">Sil</button></div></div>`).join('')}</div>`}
function newMemberCouponDialogHtml(c=null){const edit=!!c;return `<div class="memberCouponDialog autoMemberCouponDialog"><div class="memberCouponHead"><div><h2>${edit?'Yeni Üye Otomatik Kuponunu Düzenle':'Yeni Üye Otomatik Kupon'}</h2><p>Buradaki aktif kuponlar, hesap oluşturan her yeni üyeye otomatik ve bağımsız olarak atanır.</p></div><button class="smallBtn" onclick="closeMemberCouponModal()">Kapat</button></div><div class="autoCouponExisting"><h3>Otomatik Kuponlar</h3>${autoCouponTemplateRowsHtml()}</div><div class="memberCouponGrid"><div class="field memberCouponWide"><label><b>Müşteriye Gösterilecek Başlık *</b></label><input id="memberCouponHeadline" class="formControl" value="${attr(c?.headline||'')}" placeholder="Örn: Hoşgeldin Kuponu"></div><div class="field"><label><b>Kupon Kodu *</b></label><input id="memberCouponCode" class="formControl" maxlength="32" value="${attr(c?.code||'')}" placeholder="Örn: HOSGELDIN100"></div><div class="field"><label><b>İndirim Türü *</b></label><select id="memberCouponType" class="formControl" onchange="syncMemberCouponFields()"><option value="percent" ${c?.discountType!=='fixed'?'selected':''}>Yüzde (%)</option><option value="fixed" ${c?.discountType==='fixed'?'selected':''}>Sabit TL</option></select></div><div class="field"><label><b>İndirim Değeri *</b></label><input id="memberCouponValue" class="formControl" type="number" min="1" step="1" value="${attr(c?.value??'')}" placeholder="Örn: 10"></div><div class="field"><label><b>Üye Olduktan Sonra Kaç Gün İçinde Kullanılabilir? *</b></label><input id="memberCouponValidityDays" class="formControl" type="number" min="1" max="3650" step="1" value="${attr(c?.validityDays??7)}" placeholder="Örn: 7"><small>Her müşteri için süre, kendi üyelik oluşturma gününden itibaren ayrı hesaplanır.</small></div><div class="field"><label><b>Minimum Sepet Tutarı</b></label><input id="memberCouponMinCart" class="formControl" type="number" min="0" step="1" value="${attr(c?.minCartAmount??'')}" placeholder="Örn: 1000"></div><div class="field"><label><b>Diğer Kuponlarla Birlikte Kullanılabilir</b></label><select id="memberCouponStackable" class="formControl"><option value="no" ${!c?.stackable?'selected':''}>Hayır</option><option value="yes" ${c?.stackable?'selected':''}>Evet</option></select><small id="memberCouponStackableHelp"></small></div><div class="field memberCouponMaxField" id="memberCouponMaxField"><label><b>Maksimum İndirim Tutarı</b></label><input id="memberCouponMaxDiscount" class="formControl" type="number" min="0" step="1" value="${attr(c?.maxDiscountAmount??'')}" placeholder="Örn: 300"></div><div class="field memberCouponWide"><label><b>Açıklama</b></label><input id="memberCouponTitle" class="formControl" value="${attr(c?.title||'')}" placeholder="Müşterinin kupon detayında göreceği açıklama"></div></div><button class="btn" onclick="saveNewMemberAutoCoupon('${attr(c?.id||'')}',this)">${edit?'Değişiklikleri Kaydet':'Otomatik Kuponu Ekle'}</button></div>`}
async function openNewMemberAutoCouponModal(id=''){try{newMemberCouponTemplates=(await fetch('/api/admin/new-member-coupons').then(r=>r.json())).templates||[];const c=id?newMemberCouponTemplates.find(x=>String(x.id)===String(id)):null;document.querySelector('.memberCouponOverlay')?.remove();const el=document.createElement('div');el.className='memberCouponOverlay';el.innerHTML=newMemberCouponDialogHtml(c);document.body.appendChild(el);syncMemberCouponFields();el.addEventListener('click',e=>{if(e.target===el)el.remove()})}catch(e){alert('Otomatik kuponlar yüklenemedi.')}}
async function saveNewMemberAutoCoupon(id,btn){const body=memberCouponFormBody();if(!String(body.headline||'').trim())return alert('Müşteriye gösterilecek başlığı yazın.');if(!Number.isFinite(body.validityDays)||body.validityDays<1)return alert('Üye olduktan sonra kuponun kaç gün kullanılabileceğini yazın.');delete body.expiresAt;if(!confirm(id?'Bu otomatik kupon güncellensin mi?':'Bu kupon bundan sonra hesap oluşturan yeni üyelere otomatik tanımlansın mı?'))return;try{btn.disabled=true;const r=await fetch('/api/admin/new-member-coupons'+(id?'/'+encodeURIComponent(id):''),{method:id?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Otomatik kupon kaydedilemedi.');await openNewMemberAutoCouponModal()}catch(e){alert(e.message)}finally{btn.disabled=false}}
async function deleteNewMemberAutoCoupon(id){if(!confirm('Bu otomatik kupon silinsin mi? Daha önce üyelere atanmış kuponlar silinmez; yalnız yeni üyelere otomatik atama durur.'))return;try{const r=await fetch('/api/admin/new-member-coupons/'+encodeURIComponent(id),{method:'DELETE'}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Otomatik kupon silinemedi.');await openNewMemberAutoCouponModal()}catch(e){alert(e.message)}}
function memberDateRangeButtonLabel(){const f=memberFilterState.dateFrom,t=memberFilterState.dateTo;if(f&&t)return `${f} → ${t}`;if(f)return `${f} ve sonrası`;if(t)return `${t} ve öncesi`;return 'Tarih aralığı seç'}
function closeMemberDateRangePicker(){document.querySelector('.memberDatePickerOverlay')?.remove()}
function openMemberDateRangePicker(){closeMemberDateRangePicker();const el=document.createElement('div');el.className='memberDatePickerOverlay';el.innerHTML=`<div class="memberDatePickerCard"><div class="memberDatePickerHead"><b>Üye Olma Tarihi</b><button type="button" class="smallBtn" onclick="closeMemberDateRangePicker()">Kapat</button></div><p>Başlangıç ve bitiş tarihini seç. İki tarih de seçildiğinde aradaki üyeler gösterilir.</p><div class="memberDatePickerGrid"><label><span>Başlangıç</span><input id="memberRangeFrom" class="formControl" type="date" value="${attr(memberFilterState.dateFrom)}"></label><label><span>Bitiş</span><input id="memberRangeTo" class="formControl" type="date" value="${attr(memberFilterState.dateTo)}"></label></div><div class="memberDatePickerActions"><button type="button" class="smallBtn" onclick="clearMemberDateRange()">Temizle</button><button type="button" class="btn" onclick="applyMemberDateRange()">Uygula</button></div></div>`;document.body.appendChild(el);el.addEventListener('click',e=>{if(e.target===el)closeMemberDateRangePicker()})}
function applyMemberDateRange(){const from=$('#memberRangeFrom')?.value||'',to=$('#memberRangeTo')?.value||'';if(from&&to&&from>to)return alert('Başlangıç tarihi bitiş tarihinden sonra olamaz.');memberFilterState.dateFrom=from;memberFilterState.dateTo=to;closeMemberDateRangePicker();updateMemberRows();const b=document.querySelector('.memberDateRangeButton');if(b)b.textContent=memberDateRangeButtonLabel()}
function clearMemberDateRange(){memberFilterState.dateFrom='';memberFilterState.dateTo='';closeMemberDateRangePicker();updateMemberRows();const b=document.querySelector('.memberDateRangeButton');if(b)b.textContent=memberDateRangeButtonLabel()}
async function reactivateMemberAccount(userId){if(!confirm('Bu üyelik tekrar aktif edilsin mi?'))return;const r=await fetch('/api/admin/users/'+encodeURIComponent(userId)+'/reactivate',{method:'POST'}).then(x=>x.json());if(!r.ok)return alert(r.message||'Aktifleştirilemedi.');await renderMembers(true)}
async function deleteMemberAccount(userId,name){if(!confirm(`${name} müşterisi sistemden silinsin mi? Üyelik ve müşteri profil kayıtları kaldırılır; yasal sipariş kayıtları korunur.`))return;const r=await fetch('/api/admin/users/'+encodeURIComponent(userId),{method:'DELETE'}).then(x=>x.json());if(!r.ok)return alert(r.message||'Silinemedi.');await renderMembers(true)}
async function editMemberField(userId,field,current,label){const value=prompt(label+' için yeni değeri girin.',current||'');if(value===null)return;const r=await fetch('/api/admin/users/'+encodeURIComponent(userId)+'/profile',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({[field]:value})}).then(x=>x.json());if(!r.ok)return alert(r.message||'Değiştirilemedi.');await renderMembers(true)}
async function cancelMemberAccount(userId,name){if(!confirm(`${name} adlı müşterinin üyeliği iptal edilsin mi?\n\nMüşterinin mevcut oturumu kapatılır ve tekrar giriş yapamaz. Sipariş ve geçmiş kayıtları silinmez.`))return;try{const r=await fetch('/api/admin/users/'+encodeURIComponent(userId)+'/cancel',{method:'POST'}).then(x=>x.json());if(!r.ok)throw new Error(r.message||'Üyelik iptal edilemedi.');alert('Üyelik iptal edildi.');await renderMembers(true)}catch(e){alert(e.message)}}

let allCustomerRows=[],allCustomerFilter={q:'',type:'',push:'',pwa:''};
function allCustomerFiltered(){const q=String(allCustomerFilter.q||'').toLocaleLowerCase('tr-TR');return allCustomerRows.filter(c=>(!q||[c.name,c.phone,c.email].join(' ').toLocaleLowerCase('tr-TR').includes(q))&&(!allCustomerFilter.type||(allCustomerFilter.type==='member'?c.member:!c.member))&&(!allCustomerFilter.push||(allCustomerFilter.push==='open'?c.pushActive:!c.pushActive))&&(!allCustomerFilter.pwa||c.pwaStatus===allCustomerFilter.pwa))}
function renderAllCustomerRows(){const host=$('#allCustomerRows');if(!host)return;const rows=allCustomerFiltered();host.innerHTML=rows.map((c,i)=>`<div class="allCustomerRow"><b>${esc(c.name||'İsimsiz müşteri')}</b><span>${esc(c.phone||'—')}</span><span>${c.member?'Üye':'Misafir'}</span><span>${c.orderCount||0} sipariş</span><span>${esc(c.notificationStatus||'Kapalı')}</span><span>${esc(c.pwaStatus||'—')}</span><button class="smallBtn" onclick="showAllCustomerDetail(${i})">Detay</button></div>`).join('')||'<div class="panel">Filtreye uygun müşteri bulunamadı.</div>';window.__allCustomerFiltered=rows}
function setAllCustomerFilter(k,v){allCustomerFilter[k]=v;renderAllCustomerRows()}
function showAllCustomerDetail(i){const c=(window.__allCustomerFiltered||[])[i];if(!c)return;const orders=(c.orders||[]).map(o=>`<div><b>#${esc(o.id)}</b> · ${esc(o.status||'—')} · ${Number(o.total||0).toLocaleString('tr-TR')} TL · ${esc(formatDateTime(o.createdAt)||'')}</div>`).join('')||'Sipariş yok.';alert(`${c.name||'Müşteri'}
${c.email||''}
Son site girişi: ${formatDateTime(c.lastSeenAt)||'—'}
İlk PWA: ${formatDateTime(c.firstPwaAt)||'—'}
Son PWA: ${formatDateTime(c.lastPwaAt)||'—'}
Bildirim: ${c.notificationStatus||'—'}
Son bildirim: ${formatDateTime(c.lastNotificationAt)||'—'}
Toplam giriş: ${c.visitCount||0}

Siparişler:
${orders.replace(/<[^>]+>/g,'')}`)}
async function renderAllCustomers(){const r=await fetch('/api/admin/customers-all').then(x=>x.json());allCustomerRows=r.customers||[];const st=r.stats||{};$('#view').innerHTML=`<h1>Tüm Müşteriler</h1><div class="customerStats"><div><b>${st.total||0}</b><span>Toplam Müşteri</span></div><div><b>${st.members||0}</b><span>Üye</span></div><div><b>${st.guests||0}</b><span>Misafir</span></div><div><b>${st.notificationOpen||0}</b><span>Bildirim Açık</span></div><div><b>${st.pwaActive||0}</b><span>PWA Aktif</span></div><div><b>${st.pushActive||0}</b><span>Push Aktif</span></div></div><div class="allCustomerFilters"><input class="formControl" placeholder="Müşteri ara" oninput="setAllCustomerFilter('q',this.value)"><select class="formControl" onchange="setAllCustomerFilter('type',this.value)"><option value="">Üye/Misafir: Tümü</option><option value="member">Üye</option><option value="guest">Misafir</option></select><select class="formControl" onchange="setAllCustomerFilter('push',this.value)"><option value="">Bildirim: Tümü</option><option value="open">Bildirim Açık</option><option value="closed">Bildirim Kapalı</option></select><select class="formControl" onchange="setAllCustomerFilter('pwa',this.value)"><option value="">PWA: Tümü</option><option>Aktif</option><option>Uzun süredir kullanılmıyor</option><option>Pasif / kaldırılmış olabilir</option></select></div><div id="allCustomerRows" class="allCustomerList"></div>`;renderAllCustomerRows()}
async function renderNotificationSettings(){const r=await fetch('/api/admin/notification-settings').then(x=>x.json()),s=r.settings||{};const row=(key,label)=>{const x=s.statuses?.[key]||{};return `<div class="notificationSettingRow"><label><input type="checkbox" id="ns-${key}-enabled" ${x.enabled!==false?'checked':''}> <b>${label}</b></label><input class="formControl" id="ns-${key}-title" value="${attr(x.title||'SHAZ')}" placeholder="Başlık"><textarea class="formControl" id="ns-${key}-body" rows="2">${esc(x.body||'')}</textarea></div>`};$('#view').innerHTML=`<h1>Bildirim Ayarları</h1><div class="sectionTip">Sipariş durumlarına bağlı otomatik bildirimleri burada yönetebilirsin.</div>${row('new','Sipariş Alındı')}${row('prepared','Sipariş Hazırlanıyor')}${row('shipped','Kargoya Verildi')}${row('delivered','Teslim Edildi')}<button class="btn" onclick="saveNotificationSettings()">Kaydet</button><hr><h2>Manuel / Toplu Bildirim</h2><div class="grid2"><input id="manualPushTitle" class="formControl" placeholder="Başlık (isteğe bağlı)"><input id="manualPushUrl" class="formControl" value="/" placeholder="Site içi URL"><textarea id="manualPushBody" class="formControl" rows="3" placeholder="Mesaj"></textarea><button class="btn" onclick="sendManualPush()">Bildirim İzni Açık Tüm Müşterilere Gönder</button></div>`}
...
async function sendManualPush(){const title=String($('#manualPushTitle')?.value||'').trim(),body=String($('#manualPushBody')?.value||'').trim(),url=String($('#manualPushUrl')?.value||'/').trim()||'/';if(!title&&!body)return alert('Bildirim mesajı boş olamaz.');const r=await fetch('/api/admin/push/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,body,url})}).then(x=>x.json());alert(r.ok?`${r.sent||0} cihaza gönderildi.`:(r.message||'Gönderilemedi.'))}
async function renderMembers(refresh=true){
  ...
  const districts=[...new Set((members||[]).filter(u=>!memberFilterState.province||u.province===memberFilterState.province).map(u=>u.district).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'tr'));
  $('#view').innerHTML=`<h1>Üyeler</h1><div class="sectionTip">Toplam <b>${(members||[]).length}</b> üye. Filtrelenen: <b id="memberCount">${memberFiltered().length}</b>. Seçili: <b id="memberSelectedCount">${selectedMemberIds.size}</b>. Üyeyi açmadan ad, telefon, il/ilçe, adres ve toplam sipariş sayısını görebilirsin.</div><div class="memberSelectionBar"><label><input type="checkbox" onchange="toggleAllFilteredMembers(this.checked)"> Filtrelenenlerin tümünü seç</label><div class="memberCouponActionButtons"><button class="btn secondary" type="button" onclick="openNewMemberAutoCouponModal()">Yeni Üye Otomatik Kupon</button><button id="assignCouponBtn" class="btn" ${selectedMemberIds.size?'':'disabled'} onclick="openMemberCouponModal()">Kupon Tanımla</button></div></div><div class="memberAdminTools"><input class="formControl" value="${attr(memberFilterState.search)}" placeholder="İsim, telefon, e-posta veya adres ara" oninput="updateMemberFilter('search',this.value)"><select class="formControl" onchange="updateMemberFilter('province',this.value)"><option value="">Tüm iller</option>${provinces.map(x=>`<option value="${attr(x)}" ${memberFilterState.province===x?'selected':''}>${esc(x)}</option>`).join('')}</select><select class="formControl" onchange="updateMemberFilter('district',this.value)"><option value="">Tüm ilçeler</option>${districts.map(x=>`<option value="${attr(x)}" ${memberFilterState.district===x?'selected':''}>${esc(x)}</option>`).join('')}</select><div class="memberDateFilter memberDateSingle"><label>Üye Olma Tarihi</label><button class="formControl memberDateRangeButton" type="button" onclick="openMemberDateRangePicker()">${esc(memberDateRangeButtonLabel())}</button></div><select class="formControl" onchange="updateMemberFilter('sms',this.value)"><option value="">SMS izni: Tümü</option><option value="true" ${memberFilterState.sms==='true'?'selected':''}>SMS izni açık</option><option value="false" ${memberFilterState.sms==='false'?'selected':''}>SMS izni kapalı</option></select><select class="formControl" onchange="updateMemberFilter('emailMarketing',this.value)"><option value="">E-posta izni: Tümü</option><option value="true" ${memberFilterState.emailMarketing==='true'?'selected':''}>E-posta izni açık</option><option value="false" ${memberFilterState.emailMarketing==='false'?'selected':''}>E-posta izni kapalı</option></select><select class="formControl" onchange="updateMemberFilter('provider',this.value)"><option value="">Giriş yöntemi: Tümü</option><option value="password" ${memberFilterState.provider==='password'?'selected':''}>Şifre</option><option value="google" ${memberFilterState.provider==='google'?'selected':''}>Google</option><option value="apple" ${memberFilterState.provider==='apple'?'selected':''}>Apple</option></select><select class="formControl" onchange="updateMemberFilter('coupon',this.value)"><option value="">Kupon: Tümü</option><option value="yes" ${memberFilterState.coupon==='yes'?'selected':''}>Kupon tanımlı</option><option value="no" ${memberFilterState.coupon==='no'?'selected':''}>Kupon tanımsız</option></select><div class="memberSortControl"><label>Sıralama</label><select class="formControl" onchange="updateMemberFilter('sort',this.value)"><option value="newest" ${memberFilterState.sort==='newest'?'selected':''}>En yeni üyeler</option><option value="lastSeenDesc" ${memberFilterState.sort==='lastSeenDesc'?'selected':''}>Son giriş: Yeniden eskiye</option><option value="lastSeenAsc" ${memberFilterState.sort==='lastSeenAsc'?'selected':''}>Son giriş: Eskiden yeniye</option><option value="name" ${memberFilterState.sort==='name'?'selected':''}>Ada göre</option><option value="orders" ${memberFilterState.sort==='orders'?'selected':''}>Sipariş sayısına göre</option><option value="coupons" ${memberFilterState.sort==='coupons'?'selected':''}>Kupon sayısına göre</option><option value="province" ${memberFilterState.sort==='province'?'selected':''}>İl / ilçeye göre</option></select></div><button class="btn" onclick="exportMembersExcel()">Excel'e Aktar</button></div><div class="memberAdminList" id="memberRows">${memberRowsHtml()}</div>`;
}

function renderLegalDocuments(){
  const labels={MEMBERSHIP:'Üyelik Sözleşmesi',KVKK:'KVKK Aydınlatma Metni',PRIVACY:'Gizlilik Politikası',COOKIE:'Çerez Politikası',PRE_INFORMATION:'Ön Bilgilendirme Formu',DISTANCE_SALES:'Mesafeli Satış Sözleşmesi'};
  const rows=Object.entries(labels).map(([type,label])=>{const d=legalDocuments.find(x=>x.type===type)||{type,version:'1',title:label,content:'',active:true};return `<div class=panel><div class=grid2><div class=field><label><b>Başlık</b></label><input class=formControl data-legal-title="${attr(type)}" value="${attr(d.title||label)}"></div><div class=field><label><b>Versiyon</b></label><input class=formControl data-legal-version="${attr(type)}" value="${attr(d.version||'1')}" readonly><div class=help>Daha önce kabul edilmiş bir metni değiştirirsen sürüm otomatik artırılır.</div></div></div><div class=field><label><b>${label}</b></label><textarea class=formControl data-legal-content="${attr(type)}" rows=10 placeholder="Metni buraya yazın.">${esc(d.content||'')}</textarea></div></div>`}).join('');
  $('#view').innerHTML=`<h1>Hukuki Metinler</h1><div class=sectionTip>Tüm metinleri düzenledikten sonra aşağıdaki tek butona bas. Hepsi birlikte kaydedilir. Daha önce müşteri tarafından kabul edilmiş bir sürüm değiştirilirse eski kayıt korunur ve yeni sürüm otomatik oluşturulur.</div>${rows}<div class=saveBar><button class=btn onclick=saveLegalDocuments(this)>Değişiklikleri Kaydet</button></div>`;
}
async function saveLegalDocuments(btn){
  const types=['MEMBERSHIP','KVKK','PRIVACY','COOKIE','PRE_INFORMATION','DISTANCE_SALES'];const documents=types.map(type=>({type,title:document.querySelector(`[data-legal-title="${type}"]`)?.value||'',version:document.querySelector(`[data-legal-version="${type}"]`)?.value||'1',content:document.querySelector(`[data-legal-content="${type}"]`)?.value||'',active:true}));
  try{btn.disabled=true;const oldText=btn.textContent;btn.textContent='Kaydediliyor…';const r=await fetch('/api/admin/legal-documents',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({documents})}).then(r=>r.json());if(!r.ok)throw new Error(r.message||'Kaydedilemedi.');legalDocuments=r.documents||documents;alert('Tüm hukuki metinler kaydedildi.');renderLegalDocuments()}catch(e){alert(e.message)}finally{btn.disabled=false;if(document.body.contains(btn))btn.textContent='Değişiklikleri Kaydet'}
}
  ...
  `<div class="settingsHub">
    ...
    <details class="panel simpleAdminDetails settingsGroup announcementAdmin">
      <summary>Duyuru Panosu <small>Siteye girince çıkan bilgilendirme</small></summary><div class="simpleDetailsBody settingsBody">
        ...
        ${textarea('Duyuru metni','settings.siteAnnouncement.text',settings.siteAnnouncement?.text||'','Kampanya veya bilgilendirme metnini buraya yaz.','.siteAnnouncement')}<div class=field><label><b>Duyuru fotoğrafı (isteğe bağlı)</b></label><input id=siteAnnouncementImageFile type=file accept="image/*"><div class=builderSpotlightUploadRow><button type=button class=smallBtn onclick="uploadSiteAnnouncementImage()">Fotoğraf Yükle</button>${settings.siteAnnouncement?.imageUrl?'<button type=button class=smallBtn onclick="removeSiteAnnouncementImage()">Fotoğrafı Kaldır</button>':''}</div><div class=help>Hangi boyutta yüklersen yükle, müşteride popup içine sığdırılır ve oranı korunur.</div></div>
        <div class="field">
...
async function uploadSiteAnnouncementImage(){const f=document.getElementById('siteAnnouncementImageFile')?.files?.[0];if(!f)return alert('Önce bir fotoğraf seç.');const fd=new FormData();fd.append('files',f);const r=await fetch('/api/upload',{method:'POST',body:fd}).then(x=>x.json());if(!r.ok||!r.files?.[0])return alert(r.message||'Fotoğraf yüklenemedi.');settings.siteAnnouncement.imageUrl=r.files[0].url;await fetch('/api/admin/state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({settings,catalog})});changed('.siteAnnouncement');renderSite()}
async function removeSiteAnnouncementImage(){settings.siteAnnouncement.imageUrl='';await fetch('/api/admin/state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({settings,catalog})});changed('.siteAnnouncement');renderSite()}
...
function moveProductWithinCategory(productId,direction){
  ...
  catalog.recommendedProductOrder=(catalog.products||[]).map(x=>x.id);
  changed('#products');
  renderCatalog();
}
...
function productDrop(e,targetId){
  ...
  // v153: Yönetimde sürüklenen sıra müşterinin “Önerilen” sırasına da aynen yansır.
  catalog.recommendedProductOrder=products.map(p=>p.id);
...
function renderCustom(){
  ...
  `<details class="panel simpleAdminDetails"><summary>Yazı ücretleri <small>Kişiselleştirme fiyatları</small></summary><div class="grid2 simpleDetailsBody">
    ...
    ${input('3. ve sonrası','catalog.personalizationPricing.thirdPlus',50,'Üçüncü ve sonraki her ürün daima 50 TL.','.drawer','number')}
...
async function renderOrders(){
  ...
  const html=`<div class=panel><div class=orderToolbar>
    ...
    <div class=field><label><b>Durum</b></label><select id=orderStatusFilter class=formControl onchange=paintOrders()><option value=all>Tümü</option><option value=new>Yeni</option><option value=prepared>Hazırlandı</option><option value=shipped>Kargoya Verildi</option><option value=delivered>Teslim Edildi</option><option value=notexported>Excel’e Aktarılmadı</option><option value=exported>Excel’e Aktarıldı</option></select></div>
...
function orderDisplayId(o){return String(o?.dailyDisplayId||o?.id||'')}
...
function statusText(s){return s==='delivered'?'Teslim Edildi':s==='shipped'?'Kargoya Verildi':s==='prepared'?'Hazırlandı':'Yeni'}
function closeOrderEditor(){document.getElementById('orderEditOverlay')?.remove()}
function orderEditorItemRows(o){
  return (o.items||[]).map((x,i)=>`<div class=orderEditItemRow><div class=field><label><b>Ürün ${i+1} adı</b></label><input class=formControl data-order-item-name="${i}" value="${attr(x.product?.name||'Ürün')}"></div><div class=field><label><b>Fiyat</b></label><input class=formControl data-order-item-price="${i}" type=number min=0 value="${attr(Number(x.product?.price||0))}"></div><div class=field><label><b>Adet</b></label><input class=formControl data-order-item-qty="${i}" type=number min=1 step=1 value="${attr(Math.max(1,Number(x.qty||1)))}"></div></div>`).join('');
}
function openOrderEditor(id){
  closeOrderEditor();
  const o=orderCache.find(x=>String(x.id||'')===String(id));if(!o)return alert('Sipariş bulunamadı.');
  const c=o.customer||{};
  const payment=String(o.payment||'cod');
  const html=`<div class=orderEditOverlay id=orderEditOverlay onclick="if(event.target===this)closeOrderEditor()"><div class=orderEditDialog role=dialog aria-modal=true aria-label="Siparişi düzenle"><div class=orderEditHead><div><h2>${esc(orderDisplayId(o))} — Siparişi Düzenle</h2><div class=help>Bu düzenleme yalnızca bu sipariş kaydını değiştirir; ürün kataloğunu değiştirmez.</div></div><button type=button class=smallBtn onclick=closeOrderEditor()>Kapat</button></div><div class=orderEditGrid>
    <div class=field><label><b>Ad Soyad</b></label><input id=oeFullName class=formControl value="${attr(c.fullName||'')}"></div>
    <div class=field><label><b>Telefon</b></label><input id=oePhone class=formControl inputmode=tel value="${attr(c.phone||'')}"></div>
    <div class=field><label><b>2. Telefon</b></label><input id=oeExtraPhone class=formControl inputmode=tel value="${attr(c.extraPhone||'')}"></div>
    <div class=field><label><b>İl</b></label><input id=oeProvince class=formControl value="${attr(c.province||'')}"></div>
    <div class=field><label><b>İlçe</b></label><input id=oeDistrict class=formControl value="${attr(c.district||'')}"></div>
    <div class=field><label><b>Mahalle</b></label><input id=oeNeighborhood class=formControl value="${attr(c.neighborhood||'')}"></div>
    <div class=field><label><b>Cadde</b></label><input id=oeAvenue class=formControl value="${attr(c.avenue||'')}"></div>
    <div class=field><label><b>Sokak</b></label><input id=oeStreet class=formControl value="${attr(c.street||'')}"></div>
    <div class="field orderEditWide"><label><b>Tam adres / adres devamı</b></label><input id=oeFullAddress class=formControl value="${attr(c.fullAddress||'')}"></div>
    <div class=field><label><b>Bina No</b></label><input id=oeBuildingNo class=formControl value="${attr(c.buildingNo||'')}"></div>
    <div class=field><label><b>Kat</b></label><input id=oeFloor class=formControl value="${attr(c.floor||'')}"></div>
    <div class=field><label><b>Daire</b></label><input id=oeDoorNo class=formControl value="${attr(c.doorNo||'')}"></div>
    <div class=field><label><b>Şube adı</b></label><input id=oeBranchName class=formControl value="${attr(c.branchName||'')}"></div>
    <div class="field orderEditWide"><label><b>Teslimat notu</b></label><input id=oeNote class=formControl value="${attr(c.note||'')}"></div>
    <div class=field><label><b>Ödeme</b></label><select id=oePayment class=formControl><option value=cod ${payment==='cod'?'selected':''}>Kapıda ödeme</option><option value=online ${payment==='online'?'selected':''}>Online ödeme</option>${!['cod','online'].includes(payment)?`<option value="${attr(payment)}" selected>${esc(payment)}</option>`:''}</select></div>
    <div class=field><label><b>Toplam (TL)</b></label><input id=oeTotal class=formControl type=number min=0 value="${attr(Number(o.total||0))}"></div>
  </div><div class=orderEditItems><h3>Sipariş ürünleri</h3>${orderEditorItemRows(o)}</div><div class=orderEditFooter><button type=button class=smallBtn onclick=closeOrderEditor()>Vazgeç</button><button type=button class=btn onclick="saveOrderEdit('${attr(o.id)}',this)">Değişiklikleri Kaydet</button></div></div></div>`;
  document.body.insertAdjacentHTML('beforeend',html);
}
async function saveOrderEdit(id,button){
  const items=[...document.querySelectorAll('[data-order-item-name]')].map((el,i)=>({name:el.value,price:Number(document.querySelector(`[data-order-item-price="${i}"]`)?.value||0),qty:Number(document.querySelector(`[data-order-item-qty="${i}"]`)?.value||1)}));
  const body={customer:{fullName:$('#oeFullName')?.value||'',phone:$('#oePhone')?.value||'',extraPhone:$('#oeExtraPhone')?.value||'',province:$('#oeProvince')?.value||'',district:$('#oeDistrict')?.value||'',neighborhood:$('#oeNeighborhood')?.value||'',avenue:$('#oeAvenue')?.value||'',street:$('#oeStreet')?.value||'',fullAddress:$('#oeFullAddress')?.value||'',buildingNo:$('#oeBuildingNo')?.value||'',floor:$('#oeFloor')?.value||'',doorNo:$('#oeDoorNo')?.value||'',branchName:$('#oeBranchName')?.value||'',note:$('#oeNote')?.value||''},payment:$('#oePayment')?.value||'cod',total:Number($('#oeTotal')?.value||0),items};
  const old=button?.textContent||'Değişiklikleri Kaydet';if(button){button.disabled=true;button.textContent='Kaydediliyor…'}
  try{
    const r=await fetch('/api/orders/'+encodeURIComponent(id),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.message||'Sipariş düzenlenemedi.');
    orderCache=await fetch('/api/orders').then(x=>x.json());closeOrderEditor();paintOrders();
  }catch(e){alert(e.message||'Sipariş düzenlenemedi.');if(button){button.disabled=false;button.textContent=old}}
}
async function deleteAdminOrder(id){
  const o=orderCache.find(x=>String(x.id||'')===String(id));if(!o)return alert('Sipariş bulunamadı.');
  if(!confirm(`${orderDisplayId(o)} siparişi yönetim panelinden silinsin mi? Bu işlem geri alınamaz.`))return;
