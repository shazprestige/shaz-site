...
function renderCategoryHub(){
  ...
  wrap.innerHTML=cats.map(c=>`<button class="categoryFreshCard ${c.cover?'hasCover':''} ${c.allProducts?'allProductsTile':''}" onclick="chooseCategoryFromHub('${escapeAttr(c.id)}','${escapeAttr(c.name)}')">${c.cover?`<img class="categoryFreshImage" src="${escapeAttr(c.cover)}" alt="${escapeAttr(c.name)}">`:`<span class="categoryFreshPlaceholder">${c.allProducts?'TÜ':escapeHtml((c.name||'?').slice(0,1))}</span>`}<span class="categoryFreshShade" aria-hidden="true"></span><span class="categoryFreshCopy"><b>${escapeHtml(c.name)}</b><span>${c.allProducts?'Tümünü gör →':'Ürünleri gör →'}</span></span></button>`).join('');
}
