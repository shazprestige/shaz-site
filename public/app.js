...
function leaveAccountArea(){closeAccountPopover();closeDrawer();if(location.pathname!=='/')history.replaceState({...history.state,shazAccount:false},'','/');window.scrollTo({top:0,left:0,behavior:'auto'})}
...
function authPasswordField(id,placeholder,autocomplete){return `<div class="authPasswordWrap"><input id="${id}" class="formControl" type="password" autocomplete="${autocomplete}" placeholder="${placeholder}"><button type="button" class="authEye" onclick="toggleAuthPassword('${id}',this)" aria-label="Şifreyi göster">◉</button></div>`}
function toggleAuthPassword(id,btn){const x=document.getElementById(id);if(!x)return;x.type=x.type==='password'?'text':'password';btn.classList.toggle('active',x.type==='text')}
...
function protectedEmailAttrs(){return ' data-1p-ignore="true" data-lpignore="true" data-bwignore="true" data-protonpass-ignore="true" spellcheck="false" '}
...
function showRegister(updateRoute=true){closeAccountPopover();if(updateRoute)syncAccountRoute('/kayit');const body=`<div id="authFormMessage" class="authFormMessage hidden"></div><div class="authForm kigiliAuthForm"><div class="authPair"><input id="regFirst" class="formControl" autocomplete="given-name" placeholder="Ad *"><input id="regLast" class="formControl" autocomplete="family-name" placeholder="Soyad *"></div><input id="regEmail" class="formControl" type="text" inputmode="email" autocomplete="email" ${protectedEmailAttrs()} placeholder="E-posta adresi *"><input id="regPhone" class="formControl" type="tel" inputmode="tel" autocomplete="tel" placeholder="Telefon Numarası *">${authPasswordField('regPass','Şifre *','new-password')}<input id="regPass2" class="formControl" type="password" autocomplete="new-password" placeholder="Şifre Tekrar *"><div class="birthDateField"><label for="regBirth">Doğum Tarihi *</label><input id="regBirth" class="formControl" type="text" inputmode="numeric" maxlength="10" autocomplete="bday" oninput="formatBirthDateInput(this)" placeholder="gg.aa.yyyy"></div><div class="authInfoLine">ⓘ 6698 sayılı Kişisel Verilerin Korunması Kanunu kapsamında <button type="button" onclick="openLegalDocument('KVKK')">Üye Aydınlatma Metni</button>'mize buradan ulaşabilirsiniz.</div><div class="authChecks"><label class="authCheck"><input id="regMail" type="checkbox"><span>Kampanya, indirim ve fırsatlardan <b>E-posta</b> aracılığıyla haberdar olmak istiyorum.</span></label><label class="authCheck"><input id="regSms" type="checkbox"><span>Kampanya, indirim ve fırsatlardan <b>SMS</b> aracılığıyla haberdar olmak istiyorum.</span></label></div><div class="authLegalLinks"><button type="button" onclick="openLegalDocument('MEMBERSHIP')">Üyelik Sözleşmesi</button><button type="button" onclick="openLegalDocument('PRIVACY')">Gizlilik Politikası</button><button type="button" onclick="openLegalDocument('COOKIE')">Çerez Politikası</button></div><button class="btn authSubmit" onclick="registerAccount(this)">HESAP OLUŞTUR</button></div>${socialAuthButtons()}<div class="authSwitch">Zaten hesabınız var mı? <button type="button" class="authSwitchBlue" onclick="showLogin()">GİRİŞ YAP</button></div>`;openDrawer(accountAuthPage('Hesap oluştur','Devam etmek için bilgilerinizi girin. Zaten bir hesabınız varsa, <button type="button" class="authLeadLink" onclick="showLogin()">giriş yapın.</button>',body));loadPublicLegalDocuments()}
...
function showForgotPassword(updateRoute=true){if(updateRoute)syncAccountRoute('/giris');const body=`<div id="authFormMessage" class="authFormMessage hidden"></div><div class="authForm kigiliAuthForm"><input id="forgotIdentifier" class="formControl" type="text" inputmode="email" autocomplete="username" ${protectedEmailAttrs()} placeholder="E-posta veya telefon numarası"><button class="btn authSubmit" onclick="requestPasswordReset(this)">ŞİFRE SIFIRLAMA BAĞLANTISI GÖNDER</button></div><button class="authTextLink" type="button" onclick="showLogin(false)">GİRİŞ EKRANINA DÖN</button>`;openDrawer(accountAuthPage('Şifremi unuttum','Hesabınıza kayıtlı e-posta adresini veya telefon numaranızı girin.',body,'showLogin(false)'))}
...
function showResetPassword(updateRoute=true){if(updateRoute&&location.pathname!=='/sifre-sifirla')syncAccountRoute('/sifre-sifirla');const token=new URLSearchParams(location.search).get('token')||'';const body=`<div class="authForm kigiliAuthForm">${authPasswordField('resetPass','Yeni Şifre','new-password')}<input id="resetPass2" class="formControl" type="password" autocomplete="new-password" placeholder="Yeni Şifre Tekrar"><button class="btn authSubmit" onclick="finishPasswordReset(this,'${escapeAttr(token)}')">ŞİFREMİ GÜNCELLE</button></div>`;openDrawer(accountAuthPage('Yeni şifre oluştur','Yeni şifrenizi belirleyin.',body,'showLogin()'))}
...
async function showAccountOrders(updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/siparisler');try{const r=await apiJson('/api/account/orders');const rows=(r.orders||[]).map(o=>`<div class="accountOrder"><div class="accountOrderTop"><b>Sipariş #${escapeHtml(o.dailyDisplayId||o.id)}</b><span>${money(o.total)}</span></div><small>${escapeHtml(o.createdAtTR||o.createdAt||'')} · ${escapeHtml(o.status||'')}</small><div class="accountRowActions"><button class="smallBtn" onclick="showAccountOrderDetail('${escapeAttr(o.id)}')">Detay</button></div></div>`).join('')||'<div class="accountEmptyState">Henüz hesabınıza bağlı sipariş bulunmuyor.</div>';openDrawer(accountStandardPage(`<div class="accountShell"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Siparişlerim</h2></div>${rows}</div>`,'showAccountHome()'))}catch(e){alert(e.message)}}
...
async function showAccountCoupons(updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/kuponlar');try{const r=await apiJson('/api/account/coupons');openDrawer(accountStandardPage(`<div class="accountShell"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Kuponlarım</h2></div>${(r.coupons||[]).length?'<div class="panel">Kuponlarınız listelendi.</div>':'<div class="accountEmptyState">Şu anda hesabınıza tanımlı kupon bulunmuyor.</div>'}</div>`,'showAccountHome()'))}catch(e){alert(e.message)}}
function showChangePassword(updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/sifre');openDrawer(accountStandardPage(`<div class="accountShell"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Şifre Değiştir</h2></div><div class="accountForm"><input id="oldPass" class="formControl" type="password" placeholder="Mevcut şifre"><input id="newPass" class="formControl" type="password" placeholder="Yeni şifre"><input id="newPass2" class="formControl" type="password" placeholder="Yeni şifre tekrar"><button class="btn" onclick="changeAccountPassword(this)">Şifreyi Değiştir</button></div></div>`,'showAccountHome()'))}
...
function showAccountFavorites(updateRoute=true){if(!currentAccountUser)return showLogin(updateRoute);if(updateRoute)syncAccountRoute('/hesabim/favoriler');const ps=catalog.products.filter(p=>favorites.has(p.id));const content=ps.length?`<div class="memberFavoritesGrid">${ps.map(p=>{const img=mainProductImage(p);return `<div class="memberFavoriteCard"><button type="button" class="memberFavoritePhoto" onclick="openProductDetail('${p.id}','favorites')">${img?`<img src="${escapeAttr(img)}" alt="${escapeAttr(p.name||'Ürün')}">`:'⌚'}</button><div class="memberFavoriteInfo"><b>${escapeHtml(p.name)}</b><span>${money(p.price)}</span><div class="accountRowActions"><button class="smallBtn" onclick="openProductDetail('${p.id}','favorites')">Ürünü İncele</button><button class="smallBtn" onclick="removeFavoriteFromAccount('${p.id}')">Favoriden Kaldır</button></div></div></div>`}).join('')}</div>`:'<div class="accountEmptyState">Henüz favori ürününüz bulunmuyor.</div>';openDrawer(accountStandardPage(`<div class="accountShell memberFavoritesPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Favorilerim</h2></div>${content}</div>`,'showAccountHome()'))}
