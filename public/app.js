...
function savedAddressDisplay(a){return [a.neighborhood,a.avenue,a.street,a.fullAddress,a.district,a.province].filter(Boolean).join(' · ')}
function showSavedAddressCheckout(){
  checkoutState.payment=checkoutState.payment||'cod';
  ...
  const c=checkoutState.customer||{};
  const cards=currentAccountAddresses.map(a=>`<label class="savedCheckoutAddress ${a.id===preferred?'is-selected':''}"><input type="radio" name="savedCheckoutAddress" value="${escapeAttr(a.id)}" ${a.id===preferred?'checked':''} onchange="selectSavedCheckoutAddress('${escapeAttr(a.id)}')"><div class="savedCheckoutAddressHead"><b>${escapeHtml(a.title||'Adres')}</b>${a.isDefault?'<span>Varsayılan Adres</span>':''}</div><strong>${escapeHtml(a.fullName||[currentAccountUser.firstName,currentAccountUser.lastName].filter(Boolean).join(' '))}</strong><small>${escapeHtml(a.phone||currentAccountUser.phone||'')}</small><p>${escapeHtml(savedAddressDisplay(a))}</p></label>`).join('');
  openDrawer(`<div class="checkoutShell checkoutShell--address savedAddressCheckout"><div class="checkoutStickyTop addressStickyTop"><div class="checkoutTop"><div><span class="checkoutEyebrow">TESLİMAT BİLGİLERİ</span><h2>Kayıtlı adresinizden birini seçin</h2></div><button class="pill" onclick="closeDrawer()">Kapat</button></div><div class="checkoutBackRow"><button type="button" class="checkoutBackBtn" onclick="checkout()">← Sepete dön</button></div><div class="checkoutSteps"><span>1 Sepet</span><span class="active">2 Teslimat</span><span>3 Onay</span></div></div><div class="checkoutScrollBody addressScrollBody"><div class="savedCheckoutAddressList">${cards}</div><button class="savedNewAddressBtn" type="button" onclick="addressStep(true)">＋ Yeni / farklı bir adres kullan</button><div class="savedCheckoutExtras"><div class="field"><label><b>2. Telefon Numarası *</b><small class="fieldHelp">Size ulaşamazsak kullanabileceğimiz farklı bir numara.</small></label><input class="formControl" id="saved-extraPhone" type="tel" inputmode="numeric" value="${escapeAttr(c.extraPhone||'')}" placeholder="05xx xxx xx xx"></div><div class="field checkoutPaymentField"><label><b>Ödeme yöntemi *</b></label>${(()=>{const cod=settings.paymentMethods?.cod!==false,online=settings.paymentMethods?.online!==false,available=[...(cod?['cod']:[]),...(online?['online']:[])];if(!available.includes(checkoutState.payment))checkoutState.payment=available[0]||'';return `<select id="saved-pay" class="formControl">${cod?`<option value="cod" ${checkoutState.payment==='cod'?'selected':''}>Kapıda ödeme</option>`:''}${online?`<option value="online" ${checkoutState.payment==='online'?'selected':''}>Online ödeme</option>`:''}</select>`})()}</div><div class="field"><label><b>Teslimat notu</b><small class="fieldHelp">İsteğe bağlı</small></label><textarea class="formControl" id="saved-note" rows="2" placeholder="Teslimat notu">${escapeHtml(c.note||'')}</textarea></div></div></div><div class="checkoutStickyBottom addressStickyBottom"><button class="btn checkoutPrimary" onclick="useSavedAddressAndContinue()">Bilgilerimi Kontrol Et →</button></div></div>`)
}
function selectSavedCheckoutAddress(id){checkoutState.selectedAddressId=id;document.querySelectorAll('.savedCheckoutAddress').forEach(el=>el.classList.toggle('is-selected',el.querySelector('input')?.value===id))}
function useSavedAddressAndContinue(){
  ...
  const primary=normalizeTRMobile(a.phone||currentAccountUser?.phone||''),extra=normalizeTRMobile($('#saved-extraPhone')?.value||'');if(!primary.ok)return alert(phoneValidationMessage('Telefon numarası'));if(!extra.ok)return alert('Lütfen geçerli bir 2. telefon numarası girin.');if(primary.value===extra.value)return alert('İki telefon numarası aynı olamaz. Lütfen farklı bir 2. telefon numarası girin.');
...
function authEyeSvg(visible=false){return visible?'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3l18 18"/><path d="M10.6 10.7a2 2 0 0 0 2.7 2.7M9.9 4.3A10.9 10.9 0 0 1 12 4c5.5 0 9 5.4 9 8a9.6 9.6 0 0 1-2.1 3.8M6.2 6.2C4.2 7.7 3 10.1 3 12c0 2.6 3.5 8 9 8 1.5 0 2.8-.4 4-1"/></svg>':'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 12s3.5-6 9-6 9 6 9 6-3.5 6-9 6-9-6-9-6Z"/><circle cx="12" cy="12" r="2.7"/></svg>'}
function authPasswordField(id,placeholder,autocomplete,watchMode=''){const watch=watchMode?`;updateAuthSubmitState('${watchMode}')`:'';return `<div class="authPasswordWrap"><input id="${id}" class="formControl" type="password" autocomplete="${autocomplete}" placeholder="${placeholder}" oninput="flashPasswordChar(this,event)${watch}"><span class="authLastCharFlash" aria-hidden="true"></span><button type="button" class="authEye" onclick="toggleAuthPassword('${id}',this)" aria-label="Şifreyi göster">${authEyeSvg(false)}</button></div>`}
...
async function showAccountCoupons(updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/kuponlar');try{const r=await apiJson('/api/account/coupons'),rows=(r.coupons||[]);const html=rows.length?`<div class="accountCouponList">${rows.map(c=>`<div class="accountCouponCard ${c.status!=='active'?'is-muted':''}"><div><b>${escapeHtml(c.code||'')}</b><span>${escapeHtml(c.title||'Üyeye özel kupon')}</span></div><strong>${c.discountType==='fixed'?money(c.value):'%'+Number(c.value||0)}</strong><small>${c.status==='expired'?'Süresi dolmuş':c.status==='used'?'Kullanılmış':(c.expiresAt?'Son kullanım: '+new Date(c.expiresAt).toLocaleDateString('tr-TR'):'Aktif')}</small></div>`).join('')}</div>`:'<div class="accountEmptyState">Şu anda hesabınıza tanımlı kupon bulunmuyor.</div>';openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Kuponlarım</h2></div>${html}</div>`,'showAccountHome()'))}catch(e){alert(e.message)}}
...
async function logoutAccount(){try{await apiJson('/api/auth/logout',{method:'POST',body:'{}'})}catch{}currentAccountUser=null;currentAccountAddresses=[];favorites.clear();updateAccountHeader();updateFavoriteBadge();renderProducts($('#search')?.value||'');closeDrawer();history.replaceState({...history.state,shazAccount:false},'','/');window.scrollTo({top:0,left:0,behavior:'auto'});centerToast('✓ Başarıyla çıkış yapıldı')}
...
function showAccountFavorites(updateRoute=true){if(!currentAccountUser)return showLogin(updateRoute);if(updateRoute)syncAccountRoute('/hesabim/favoriler');const ps=catalog.products.filter(p=>favorites.has(p.id));const content=ps.length?`<div class="memberFavoritesGrid">${ps.map(p=>{const img=mainProductImage(p);return `<div class="memberFavoriteCard"><button type="button" class="memberFavoritePhoto" onclick="openProductDetail('${p.id}','favorites')">${img?`<img src="${escapeAttr(img)}" alt="${escapeAttr(p.name||'Ürün')}">`:'⌚'}</button><div class="memberFavoriteInfo"><b>${escapeHtml(p.name)}</b><span>${money(p.price)}</span><div class="accountRowActions"><button class="smallBtn" onclick="openProductDetail('${p.id}','favorites')">Ürünü İncele</button><button class="smallBtn" onclick="removeFavoriteFromAccount('${p.id}')">Favoriden Kaldır</button></div></div></div>`}).join('')}</div>`:'<div class="accountEmptyState">Henüz favori ürününüz bulunmuyor.</div>';openDrawer(accountStandardPage(`<div class="accountShell memberFavoritesPage accountCenteredSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Favorilerim</h2></div>${content}</div>`,'showAccountHome()'))}
