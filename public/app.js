...
async function init(){
  ...
  apply(); renderCampaignCards(); renderCategories(); renderProducts(); bindCore(); updateFavoriteBadge(); updateCart(); bindFloatingContacts(); renderSiteAnnouncement(); initFooter(); initCookieConsent(); registerShazPwa(); finishSiteBoot(); ensureAccountStateReady().catch(()=>{});
...
function finishSiteBoot(){clearTimeout(window.__shazSiteBootTimer);requestAnimationFrame(()=>requestAnimationFrame(()=>document.documentElement.classList.remove('siteBooting')))}
function bindCore(){
...
function categoryHubHead(title,subtitle=''){
  ...
  top.innerHTML=`<button class="categoryBack" id="categoryHubClose">←</button><div><b>${escapeHtml(title)}</b>${subtitle?`<small>${escapeHtml(subtitle)}</small>`:''}</div>`;
...
function renderCategoryHub(){
  ...
  categoryHubHead('Tüm Kategoriler','');
...
function renderProducts(filter=''){
  ...
  const cards=list.length?list.map(p=>{
    ...
    return `<a class="card productCardLink" data-product-id="${escapeAttr(p.id)}" href="/urun/${encodeURIComponent(productSlug(p))}" onclick="event.preventDefault();openProductDetail('${p.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openProductDetail('${p.id}')}"><div class="photo" data-preview-field="photo">${mainProductImage(p)?`<img src="${escapeAttr(mainProductImage(p))}" alt="${escapeAttr(name)}">`:'⌚'}${p.soldOutEnabled?soldOutVisualBadgeHtml(p,false):(p.badge?`<span class="badge badge-${badgeColor} ${p.badgeShape==='rect'?'badge-rect':''}" data-preview-field="badge"><span class="badgeText">${escapeHtml(p.badge)}</span></span>`:'')}${shippingRibbon}<button class="fav" data-preview-field="favorite" onclick="toggleFav('${p.id}',event)">${favorites.has(p.id)?'♥':'♡'}</button></div><div class="info"><h3 data-preview-field="name">${escapeHtml(name)}</h3>${desc}<div class="price"><span data-preview-field="price">${money(p.price)}</span> ${p.oldPrice?`<span class="old" data-preview-field="oldPrice">${money(p.oldPrice)}</span>`:''}</div></div></a>`;
...
function toggleFav(id,e){e?.preventDefault();e?.stopPropagation();favorites.has(id)?favorites.delete(id):favorites.add(id);localStorage.setItem('shazFavs',JSON.stringify([...favorites]));updateFavoriteBadge();renderProducts($('#search')?.value||'')}
...
function finalizeProductDetailClose(source=activeProductDetailSource||'catalog',restoreY=activeProductDetailScrollY){
  ...
  if(String(source||'').startsWith('accountOrder:')){showAccountOrderDetail(String(source).slice('accountOrder:'.length),false);return}
  closeDrawer();
...
function openProductDetail(id,source='catalog'){
  ...
  const closeLabel=(source==='favorites'||source==='accountFavorites')?'← Favorilere Dön':source==='cart'?'Sepete Dön':String(source||'').startsWith('accountOrder:')?'← Siparişe Dön':'Kapat';
...
function singleWalletPhotoStep(id){
  ...
  openDrawer(`<div class=wizardHead><button class="pill backPill" onclick='startSingleWizard(catalog.products.find(x=>x.id===${JSON.stringify(p.id)}))'>← Geri</button><div><div class=wizardProgress>Cüzdan kişiselleştirme</div><h2>${p.name}</h2></div><button class=pill onclick=closeDrawer()>Kapat</button></div>
    ...
    <button class="btn walletPhotoCartBtn" id=walletPhotoSubmit onclick='finishSingleWalletPhoto(${JSON.stringify(p.id)},this)'>Fotoğrafı Yükle ve Sepete Ekle</button>`);
}
...
function focusCheckoutCouponPanel(){requestAnimationFrame(()=>requestAnimationFrame(()=>{const scroller=document.querySelector('.drawer.drawerCartMode .checkoutScrollBody'),panel=scroller?.querySelector('.checkoutCouponPanel');if(scroller&&panel)scroller.scrollTo({top:Math.max(0,panel.offsetTop-4),left:0,behavior:'auto'})}))}
function openCheckoutCouponPanel(){checkoutState.couponPanelOpen=true;checkout(null);focusCheckoutCouponPanel()}
function toggleCheckoutCouponPanel(){const opening=!checkoutState.couponPanelOpen;checkoutState.couponPanelOpen=opening;if(opening){checkout(null);focusCheckoutCouponPanel()}else checkout(document.querySelector('.checkoutScrollBody')?.scrollTop||0)}
...
function focusDrawerField(el){
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    if(el.id?.startsWith('addr-')){el.scrollIntoView({block:'nearest',behavior:'auto'});setTimeout(repositionSavedAddressSuggestions,40)}else if(r.top<topGuard||r.bottom>vh-bottomGuard)el.scrollIntoView({block:'nearest',behavior:'auto'});
  }));
}
...
function ensureDirectInputFocus(el){if(!el)return;try{el.focus({preventScroll:true})}catch(_){try{el.focus()}catch(__){}}}
function ensureAddressFieldFocus(el){ensureDirectInputFocus(el)}
function addrInputInner(label,id,value='',type='text',placeholder=''){
  const suggest=currentAccountUser&&checkoutState.manualAddressFromSaved?` data-address-suggest="${escapeAttr(id)}" onfocus="queueSavedAddressSuggestions(this,'${escapeAttr(id)}')" oninput="updateSavedAddressSuggestions(this,'${escapeAttr(id)}')"`:'';
...
function addressSuggestionKey(value,field){let v=String(value||'').trim();if(field==='phone'||field==='extraPhone')return v.replace(/\D/g,'');return v.toLocaleLowerCase('tr-TR').replace(/\s+/g,' ')}
function matchingAddressSuggestionValues(field,query=''){const q=addressSuggestionKey(query,field);return addressSuggestionValues(field).filter(v=>!q||addressSuggestionKey(v,field).startsWith(q))}
let activeSavedAddressSuggestionInput=null,activeSavedAddressSuggestionField='',savedAddressSuggestionTimer=0;
function closeSavedAddressSuggestions(){clearTimeout(savedAddressSuggestionTimer);document.querySelector('.savedAddressSuggestionMenu')?.remove();activeSavedAddressSuggestionInput=null;activeSavedAddressSuggestionField=''}
function positionSavedAddressSuggestions(box,input){if(!box||!input||!document.body.contains(input)||activeSavedAddressSuggestionInput!==input){closeSavedAddressSuggestions();return}const r=input.getBoundingClientRect(),vv=window.visualViewport,gap=5,viewportTop=Math.max(0,vv?.offsetTop||0),viewportH=Math.max(180,vv?.height||window.innerHeight),viewportBottom=viewportTop+viewportH,width=Math.max(160,Math.min(r.width,window.innerWidth-16));box.style.width=width+'px';box.style.left=Math.max(8,Math.min(r.left,window.innerWidth-width-8))+'px';box.style.maxHeight='210px';const natural=Math.min(box.scrollHeight||180,210),spaceBelow=Math.max(0,viewportBottom-r.bottom-gap-6),spaceAbove=Math.max(0,r.top-viewportTop-gap-6),placeBelow=spaceBelow>=Math.min(96,natural)||spaceBelow>=spaceAbove;const available=Math.max(70,Math.min(210,placeBelow?spaceBelow:spaceAbove));box.style.maxHeight=available+'px';const h=Math.min(box.scrollHeight||natural,available);const top=placeBelow?Math.min(viewportBottom-h-6,r.bottom+gap):Math.max(viewportTop+6,r.top-gap-h);box.style.top=Math.round(top)+'px';box.classList.toggle('is-above',!placeBelow)}
function queueSavedAddressSuggestions(input,field){if(!input||!checkoutState.manualAddressFromSaved)return;clearTimeout(savedAddressSuggestionTimer);savedAddressSuggestionTimer=setTimeout(()=>{if(!document.body.contains(input)||document.activeElement!==input)return;openSavedAddressSuggestions(input,field)},0);setTimeout(()=>{const b=document.querySelector('.savedAddressSuggestionMenu');if(b&&activeSavedAddressSuggestionInput===input)positionSavedAddressSuggestions(b,input)},40)}
function updateSavedAddressSuggestions(input,field){if(!input||document.activeElement!==input)return;openSavedAddressSuggestions(input,field)}
function openSavedAddressSuggestions(input,field){if(!input||!checkoutState.manualAddressFromSaved||!document.body.contains(input))return;const values=matchingAddressSuggestionValues(field,input.value);document.querySelector('.savedAddressSuggestionMenu')?.remove();if(!values.length){activeSavedAddressSuggestionInput=null;activeSavedAddressSuggestionField='';return}const box=document.createElement('div');box.className='savedAddressSuggestionMenu';box.dataset.suggestionField=field;box.innerHTML=values.map(v=>`<button type="button" onpointerdown="event.preventDefault();event.stopPropagation()" data-value="${escapeAttr(v)}">${escapeHtml(v)}</button>`).join('');box.querySelectorAll('button').forEach(b=>b.onclick=e=>{e.preventDefault();input.value=b.dataset.value||'';closeSavedAddressSuggestions();ensureDirectInputFocus(input)});document.body.appendChild(box);activeSavedAddressSuggestionInput=input;activeSavedAddressSuggestionField=field;positionSavedAddressSuggestions(box,input);requestAnimationFrame(()=>positionSavedAddressSuggestions(box,input));setTimeout(()=>{const live=document.querySelector('.savedAddressSuggestionMenu');if(live&&activeSavedAddressSuggestionInput===input)positionSavedAddressSuggestions(live,input)},120)}
function repositionSavedAddressSuggestions(){const b=document.querySelector('.savedAddressSuggestionMenu');if(b&&activeSavedAddressSuggestionInput)positionSavedAddressSuggestions(b,activeSavedAddressSuggestionInput)}
if(!window._shazAddressSuggestionBound){window._shazAddressSuggestionBound=true;window.addEventListener('resize',repositionSavedAddressSuggestions,{passive:true});window.addEventListener('scroll',repositionSavedAddressSuggestions,{passive:true,capture:true});window.visualViewport?.addEventListener('resize',repositionSavedAddressSuggestions,{passive:true});window.visualViewport?.addEventListener('scroll',repositionSavedAddressSuggestions,{passive:true});document.addEventListener('pointerdown',e=>{const b=document.querySelector('.savedAddressSuggestionMenu');if(b&&!b.contains(e.target)&&e.target!==activeSavedAddressSuggestionInput)closeSavedAddressSuggestions()},true)}
...
async function openLegalDocument(type){
  if(!publicLegalDocuments.length)await loadPublicLegalDocuments();
...
async function finalizeOrder(personalApproved,button){
  ...
  try{
    ...
    if(currentAccountUser){
      Promise.all([apiJson('/api/account/addresses'),apiJson('/api/account/coupons'),apiJson('/api/account/orders')]).then(([a,c,o])=>{
        currentAccountAddresses=Array.isArray(a.addresses)?a.addresses:[];
        currentAccountCoupons=Array.isArray(c.coupons)?c.coupons:[];
        currentAccountOrders=Array.isArray(o.orders)?o.orders:[];
...
function renderBuilderFinalSummary(){
  ...
  openDrawer(`<div class=wizardHead><button class="pill backPill" onclick=builderBackFromFinalSummary()>← Geri</button><div><div class=wizardProgress>Son Kontrol</div><h2>Kendi Setiniz Hazır</h2></div><button class=pill onclick=closeDrawer()>Kapat</button></div>
    ...
    <button class="btn builderAddCartBtn" onclick=builderAddToCart()>Setimi Sepete Ekle</button>`);
}
...
async function loadAccountState(){
  try{
    ...
    if(currentAccountUser){
      ...
      const [a,f,c,o]=await Promise.all([apiJson('/api/account/addresses'),apiJson('/api/account/favorites'),apiJson('/api/account/coupons'),apiJson('/api/account/orders')]);
      ...
      currentAccountAddresses=[];currentAccountCoupons=[];currentAccountOrders=[];favorites.clear();setAccountHeaderHint(null);updateCouponNotificationBadges()
    }
    updateAccountHeader();updateFavoriteBadge();renderProducts($('#search')?.value||'');openAccountRouteIfNeeded();
  }catch(_){currentAccountUser=null;currentAccountAddresses=[];currentAccountCoupons=[];currentAccountOrders=[];updateAccountHeader();updateCouponNotificationBadges();openAccountRouteIfNeeded();finishAccountRouteBoot()}
...
async function replayAccountPopoverSwitch(){await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))}
async function openAccountEntry(){const pop=document.querySelector('.accountEntryPopover');if(pop){if(pop.dataset.trigger==='account'){pop.remove();return}pop.remove();await replayAccountPopoverSwitch()}const drawer=$('#drawer');if(drawer&&!drawer.classList.contains('hidden')&&drawer.classList.contains('drawerAccountMode')){leaveAccountArea();return}if(!accountStateReady)await ensureAccountStateReady();currentAccountUser?showAccountHome():showAuthChoice('account')}
async function toggleFavoritesEntry(){const pop=document.querySelector('.accountEntryPopover');if(pop){if(pop.dataset.trigger==='favorites'){pop.remove();return}pop.remove();await replayAccountPopoverSwitch()}const drawer=$('#drawer');if(drawer&&!drawer.classList.contains('hidden')&&drawer.dataset.view==='favorites-main'){closeDrawer();return}if(!accountStateReady)await ensureAccountStateReady();currentAccountUser?showFavorites():showAuthChoice('favorites')}
function showAuthChoice(trigger='account'){
  closeAccountPopover();
  const el=document.createElement('div');el.className='accountEntryPopover accountEntryPopoverReplay';el.dataset.trigger=trigger;
...
function authPasswordField(id,placeholder,autocomplete,watchMode=''){const watch=watchMode?`;updateAuthSubmitState('${watchMode}')`:'';return `<div class="authPasswordWrap"><input id="${id}" type="password" class="formControl" autocomplete="${autocomplete}" placeholder="${placeholder}" oninput="${watch.replace(/^;/,'')}"><button type="button" tabindex="-1" class="authEye" onpointerdown="handleAuthEyePointer(event,'${id}',this)" onclick="event.preventDefault();event.stopPropagation()" aria-label="Şifreyi göster">${authEyeSvg(false)}</button></div>`}
function handleAuthEyePointer(e,id,btn){e.preventDefault();e.stopPropagation();const x=document.getElementById(id);if(!x)return;toggleAuthPassword(id,btn);requestAnimationFrame(()=>{try{x.focus({preventScroll:true})}catch(_){try{x.focus()}catch(__){}}})}
function toggleAuthPassword(id,btn){const x=document.getElementById(id);if(!x)return;let start=null,end=null;try{start=x.selectionStart;end=x.selectionEnd}catch(_){}const cssMask=x.dataset.passwordMask==='css',visible=cssMask?!x.classList.contains('is-visible'):x.type==='password';if(cssMask)x.classList.toggle('is-visible',visible);else if((visible&&x.type!=='text')||(!visible&&x.type!=='password'))x.type=visible?'text':'password';btn.classList.toggle('active',visible);btn.innerHTML=authEyeSvg(visible);btn.setAttribute('aria-label',visible?'Şifreyi gizle':'Şifreyi göster');const wrap=x.parentElement,flash=wrap?.querySelector('.authLastCharFlash');if(flash){flash.classList.remove('show');clearTimeout(flash._hideTimer)}wrap?.classList.remove('maskFlashActive');try{if(start!==null&&end!==null)x.setSelectionRange(start,end)}catch(_){}}
...
function showAccountHome(updateRoute=true){
  ...
  const body=`<div class="accountShell accountShellMenu"><div class="accountHero accountMenuHero"><div><h2>Merhaba ${escapeHtml(currentAccountUser.firstName||'')}</h2></div></div><div class="accountMenuList"><button class="accountMenuRow" onclick="this.blur();showAccountOrders()"><span class="accountMenuIcon">${accountMenuIcon('orders')}</span><b>Siparişlerim</b><span class="accountMenuArrow">›</span></button><button class="accountMenuRow" onclick="this.blur();showAccountProfile()"><span class="accountMenuIcon">${accountMenuIcon('profile')}</span><b>Hesap Bilgilerim</b><span class="accountMenuArrow">›</span></button><button class="accountMenuRow" onclick="this.blur();showAccountAddresses()"><span class="accountMenuIcon">${accountMenuIcon('address')}</span><b>Adreslerim</b><span class="accountMenuArrow">›</span></button><button class="accountMenuRow" onclick="this.blur();showAccountFavorites()"><span class="accountMenuIcon">${accountMenuIcon('favorite')}</span><b>Favorilerim</b><span class="accountMenuArrow">›</span></button><button class="accountMenuRow" onclick="this.blur();showAccountCoupons()"><span class="accountMenuIcon">${accountMenuIcon('coupon')}</span><b>Kuponlarım</b><span class="accountMenuNoticeBadge ${couponCount?'':'hidden'}" data-coupon-notice-badge>${couponCount||''}</span><span class="accountMenuArrow">›</span></button><div class="accountMenuDivider"></div><button class="accountMenuRow" onclick="this.blur();showChangePassword()"><span class="accountMenuIcon">${accountMenuIcon('password')}</span><b>Şifre Değiştir</b><span class="accountMenuArrow">›</span></button><button class="accountMenuRow" onclick="confirmLogoutAccount()"><span class="accountMenuIcon">${accountMenuIcon('logout')}</span><b>Çıkış Yap</b><span class="accountMenuArrow">›</span></button></div></div>`;
...
async function showAccountProfile(updateRoute=true){if(updateRoute)syncAccountRoute('/hesabim/bilgiler');const u=currentAccountUser,pending=[u.pendingEmailChange?`<small><b>Bekleyen e-posta doğrulaması:</b> ${escapeHtml(u.pendingEmailChange.email||'')}</small>`:'',u.pendingPhoneChange?`<small><b>Bekleyen telefon doğrulaması:</b> ${escapeHtml(displayAccountPhone(u.pendingPhoneChange.phone||''))}</small><div class="pendingPhoneVerify"><input id="pendingPhoneCode" class="formControl" inputmode="numeric" maxlength="8" placeholder="Doğrulama kodu"><button type="button" class="smallBtn" onclick="verifyPendingPhoneChange(this)">Telefonu Doğrula</button></div>`:''].filter(Boolean).join('');openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Hesap Bilgilerim</h2></div><div id="authFormMessage" class="authFormMessage hidden"></div>${pending?`<div class="accountProfileRules accountPendingOnly">${pending}</div>`:''}<div class="accountForm"><div class="authPair"><label class="accountProfileField"><span>Ad</span><input id="accFirst" class="formControl" value="${escapeAttr(u.firstName||'')}" placeholder="Ad *"></label><label class="accountProfileField"><span>Soyad</span><input id="accLast" class="formControl" value="${escapeAttr(u.lastName||'')}" placeholder="Soyad *"></label></div><label class="accountProfileField"><span>E-posta</span>${authInputShield(`<input id="accEmail" class="formControl" type="text" inputmode="email" ${protectedEmailAttrs()} value="${escapeAttr(u.email||'')}" placeholder="E-posta *">`)}<small>E-posta değişikliğinde yeni adresinize doğrulama bağlantısı gönderilir. Bağlantı doğrulanmadan mevcut e-posta adresiniz aktif kalır.</small></label><label class="accountProfileField"><span>Telefon</span><input id="accPhone" class="formControl" type="tel" value="${escapeAttr(displayAccountPhone(u.phone||''))}" placeholder="Telefon *"><small>Yeni telefon numarası doğrulandıktan sonra aktif olur; doğrulama tamamlanana kadar mevcut numaranız korunur.</small></label><div class="birthDateField accountProfileField"><label for="accBirth">Doğum Tarihi *</label><input id="accBirth" class="formControl" type="text" inputmode="numeric" maxlength="10" value="${escapeAttr(birthDateToTr(u.birthDate||''))}" oninput="formatBirthDateInput(this)" placeholder="gg.aa.yyyy"><small>Doğum tarihinizi hesabınızdan yalnızca bir kez değiştirebilirsiniz.</small></div><label class="authCheck"><input id="accMail" type="checkbox" ${u.emailMarketingConsent?'checked':''} onclick="return handleMarketingConsentClick(event,this,'email')"><span>Kampanya, indirim ve fırsatlardan E-posta aracılığıyla haberdar olmak istiyorum.</span></label><label class="authCheck"><input id="accSms" type="checkbox" ${u.smsMarketingConsent?'checked':''} onclick="return handleMarketingConsentClick(event,this,'sms')"><span>Kampanya, indirim ve fırsatlardan SMS aracılığıyla haberdar olmak istiyorum.</span></label><button class="btn" onclick="saveAccountProfile(this)">Kaydet</button></div></div>`,'showAccountHome()'))}
...
let pendingAccountProfileSave=null,pendingMarketingConsent=null;
function profileConfirmOverlay(title,lines,onConfirm){document.querySelector('.profileConfirmOverlay')?.remove();const el=document.createElement('div');el.className='profileConfirmOverlay';el.innerHTML=`<div class="profileConfirmCard"><h3>${escapeHtml(title)}</h3><div class="profileConfirmText">${lines.map(x=>`<p>${escapeHtml(x)}</p>`).join('')}</div><label class="profileConfirmAccept"><input type="checkbox" id="profileConfirmAccept"> <span>Onaylıyorum</span></label><div class="profileConfirmActions"><button type="button" class="pill" onclick="this.closest('.profileConfirmOverlay').remove()">Vazgeç</button><button type="button" class="btn" id="profileConfirmContinue">Devam Et</button></div></div>`;document.body.appendChild(el);el.querySelector('#profileConfirmContinue').onclick=()=>{if(!el.querySelector('#profileConfirmAccept')?.checked)return checkoutCouponMessage('Devam etmek için “Onaylıyorum” kutusunu işaretleyin.');el.remove();onConfirm?.()}}
function handleMarketingConsentClick(e,el,channel){const was=channel==='sms'?!!currentAccountUser?.smsMarketingConsent:!!currentAccountUser?.emailMarketingConsent;if(!was)return true;if(el.dataset.optoutConfirmed==='1'){delete el.dataset.optoutConfirmed;return true}e.preventDefault();el.checked=true;const text=channel==='sms'?'SMS iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları SMS aracılığıyla alamazsınız.':'E-posta iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları e-posta aracılığıyla alamazsınız.';profileConfirmOverlay('İletişim izni değişikliği',[text],()=>{el.dataset.optoutConfirmed='1';el.checked=false});return false}
function accountProfileDraft(){return {firstName:String($('#accFirst')?.value||'').trim(),lastName:String($('#accLast')?.value||'').trim(),email:String($('#accEmail')?.value||'').trim(),phone:String($('#accPhone')?.value||'').trim(),birthDate:birthDateToIso($('#accBirth')?.value),smsMarketingConsent:!!$('#accSms')?.checked,emailMarketingConsent:!!$('#accMail')?.checked}}
function profileChangedFieldIds(body,u=currentAccountUser||{}){const ids=[];if(body.firstName!==String(u.firstName||'')||body.lastName!==String(u.lastName||''))ids.push('accFirst','accLast');if(String(body.email||'').trim().toLocaleLowerCase('tr-TR')!==String(u.email||'').trim().toLocaleLowerCase('tr-TR'))ids.push('accEmail');if(displayAccountPhone(body.phone)!==displayAccountPhone(u.phone||''))ids.push('accPhone');if(body.birthDate!==String(u.birthDate||''))ids.push('accBirth');return ids}
function resetBlockedProfileFields(kind,u=currentAccountUser||{}){if(kind==='name'){if($('#accFirst'))$('#accFirst').value=u.firstName||'';if($('#accLast'))$('#accLast').value=u.lastName||''}if(kind==='phone'&&$('#accPhone'))$('#accPhone').value=displayAccountPhone(u.phone||'');if(kind==='birth'&&$('#accBirth'))$('#accBirth').value=birthDateToTr(u.birthDate||'')}
async function commitAccountProfileSave(){const pending=pendingAccountProfileSave;if(!pending)return;pendingAccountProfileSave=null;const {body,btn}=pending;try{btn.disabled=true;const r=await apiJson('/api/account/profile',{method:'PATCH',body:JSON.stringify(body)});currentAccountUser=r.user;updateAccountHeader();toast(r.message||'✓ Hesap bilgileri güncellendi');showAccountProfile(false)}catch(e){const msg=String(e.message||'Bilgiler güncellenemedi.'),ids=profileChangedFieldIds(body);if(msg.includes('Ad / Soyad'))resetBlockedProfileFields('name');if(msg.includes('Telefon numarası 7 günde'))resetBlockedProfileFields('phone');if(msg.includes('Doğum tarihinizi kullanıcı hesabından yalnızca 1 kez'))resetBlockedProfileFields('birth');showAuthErrors(msg,ids.length?ids:['accEmail'])}finally{if(btn)btn.disabled=false}}
async function saveAccountProfile(btn){['accFirst','accLast','accEmail','accPhone','accBirth'].forEach(id=>clearAuthFieldError(document.getElementById(id)));const required=['accFirst','accLast','accEmail','accPhone','accBirth'],missing=required.filter(id=>!String(document.getElementById(id)?.value||'').trim());if(missing.length)return showAuthErrors('Lütfen kırmızı işaretli zorunlu alanları doldurun.',missing);const body=accountProfileDraft();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email))return showAuthErrors('Lütfen geçerli bir e-posta adresi girin.',['accEmail']);if(String(body.phone||'').replace(/\D/g,'').length<10)return showAuthErrors('Lütfen geçerli bir telefon numarası girin.',['accPhone']);if(!body.birthDate)return showAuthErrors('Doğum tarihini gg.aa.yyyy şeklinde girin.',['accBirth']);const u=currentAccountUser||{},nameChanged=body.firstName!==String(u.firstName||'')||body.lastName!==String(u.lastName||''),phoneChanged=displayAccountPhone(body.phone)!==displayAccountPhone(u.phone||''),emailChanged=body.email.toLocaleLowerCase('tr-TR')!==String(u.email||'').trim().toLocaleLowerCase('tr-TR'),birthChanged=body.birthDate!==String(u.birthDate||''),smsChanged=body.smsMarketingConsent!==!!u.smsMarketingConsent,mailChanged=body.emailMarketingConsent!==!!u.emailMarketingConsent;if(!nameChanged&&!phoneChanged&&!emailChanged&&!birthChanged&&!smsChanged&&!mailChanged)return toast('Değişiklik bulunmuyor.');const rules=u.profileRules||{};if(nameChanged&&Number(rules.nameWaitMs||0)>0){const msg=`Ad / Soyad bilginizi tekrar değiştirebilmeniz için ${profileWaitText(rules.nameWaitMs)} kaldı.`;resetBlockedProfileFields('name',u);return showAuthErrors(msg,['accFirst','accLast'])}if(phoneChanged&&Number(rules.phoneWaitMs||0)>0){const msg=`Telefon numaranızı tekrar değiştirebilmeniz için ${profileWaitText(rules.phoneWaitMs)} kaldı.`;resetBlockedProfileFields('phone',u);return showAuthErrors(msg,['accPhone'])}if(birthChanged&&rules.birthDateChangeAvailable===false){resetBlockedProfileFields('birth',u);return showAuthErrors('Doğum tarihinizi hesabınızdan daha önce değiştirdiniz. Bundan sonraki değişiklik yalnızca yönetim panelinden yapılabilir.',['accBirth'])}const warnings=[];if(nameChanged)warnings.push('Ad / Soyad bilgisi kaydedildikten sonra 30 gün boyunca tekrar değiştirilemez.');if(phoneChanged)warnings.push('Yeni telefon numarası doğrulanmadan mevcut telefon numaranız aktif kalır. Doğrulama tamamlandıktan sonra telefon değişikliği 7 gün boyunca tekrar yapılamaz.');if(emailChanged)warnings.push('Yeni e-posta adresi doğrulanmadan mevcut e-posta adresiniz aktif kalır.');if(birthChanged)warnings.push('Doğum tarihinizi hesabınızdan yalnızca bir kez değiştirebilirsiniz.');if(smsChanged&&u.smsMarketingConsent&&!body.smsMarketingConsent&&$('#accSms')?.dataset.optoutConfirmed!=='1')warnings.push('SMS iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları SMS aracılığıyla alamazsınız.');if(mailChanged&&u.emailMarketingConsent&&!body.emailMarketingConsent&&$('#accMail')?.dataset.optoutConfirmed!=='1')warnings.push('E-posta iznini kapatırsanız kampanya, indirim, fırsat ve size özel duyuruları e-posta aracılığıyla alamazsınız.');if(!warnings.length){pendingAccountProfileSave={body,btn};return commitAccountProfileSave()}pendingAccountProfileSave={body,btn};profileConfirmOverlay('Hesap bilgisi değişikliği',warnings,commitAccountProfileSave)}
...
function accountOrdersRowsHtml(rows=[]){return (rows||[]).map(o=>`<div class="accountOrder"><div class="accountOrderTop"><b>Sipariş #${escapeHtml(o.dailyDisplayId||o.id)}</b><span class="accountOrderTopTotal">${money(o.total)}</span></div><small>${escapeHtml(o.createdAtTR||o.createdAt||'')}</small>${accountOrderThumbs(o.items||[])}<div class="accountOrderListStatus">${escapeHtml(accountOrderStatusLabel(o.status))}</div><div class="accountRowActions"><button class="smallBtn" onclick="showAccountOrderDetail('${escapeAttr(o.id)}')">Detay</button></div></div>`).join('')||'<div class="accountEmptyState">Henüz hesabınıza bağlı sipariş bulunmuyor.</div>'}
function renderAccountOrdersPage(rows=[]){openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Siparişlerim</h2></div>${accountOrdersRowsHtml(rows)}</div>`,'showAccountHome()'))}
async function showAccountOrders(updateRoute=true){try{document.activeElement?.blur()}catch(_){}if(updateRoute)syncAccountRoute('/hesabim/siparisler');renderAccountOrdersPage(currentAccountOrders);try{const r=await apiJson('/api/account/orders');currentAccountOrders=Array.isArray(r.orders)?r.orders:[];if(location.pathname==='/hesabim/siparisler')renderAccountOrdersPage(currentAccountOrders)}catch(e){console.warn('Siparişler yenilenemedi:',e.message)}}
...
function accountOrderItemDetailHtml(x,orderId=''){const img=accountOrderItemImage(x),qty=Number(x.qty||1),price=Number(x.product?.price||0),pid=x.product?.id||'';return `<div class="accountOrderItem ${pid?'is-clickable':''}" ${pid?`role="button" tabindex="0" onclick="openProductDetail('${escapeAttr(pid)}','accountOrder:${escapeAttr(orderId)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openProductDetail('${escapeAttr(pid)}','accountOrder:${escapeAttr(orderId)}')}"`:''}><div class="accountOrderItemMain">${img?`<div class="accountOrderItemImage"><img src="${escapeAttr(img)}" alt="${escapeAttr(x.product?.name||'Ürün')}"></div>`:''}<div class="accountOrderItemCopy"><b>${escapeHtml(x.product?.name||'Ürün')}</b><span>Adet: ${qty}</span><strong>${money(price*qty)}</strong></div></div>${accountOrderPersonalizationHtml(x)}</div>`}
...
function maybeShowCouponAnnouncement(){
  ...
  const heading=rows.length===1?String(rows[0].headline||'Kupon').trim():'Kuponlarınız';
  const el=document.createElement('div');el.className='couponNoticeOverlay';
  el.innerHTML=`<div class="couponNoticeCard"><button type="button" class="couponNoticeClose" onclick="this.closest('.couponNoticeOverlay').remove()">×</button><h3>${escapeHtml(heading)}</h3><div class="couponNoticeList">${rows.map(c=>{const ex=couponExpiryInfo(c);return `<div class="couponNoticeItem">${rows.length>1&&String(c.headline||'').trim()?`<h4 class="couponNoticeItemHeadline">${escapeHtml(c.headline)}</h4>`:''}<div class="couponNoticeCode"><b>${escapeHtml(c.code||'')}</b><button type="button" class="couponCopyBtn" onclick="copyCouponCode('${escapeAttr(c.code||'')}',this)" aria-label="Kupon kodunu kopyala">${couponCopyIcon()}</button></div><strong>${escapeHtml(couponDiscountLabel(c))}</strong>${String(c.title||'').trim()?`<p>${escapeHtml(c.title)}</p>`:''}${couponRulesHtml(c,'couponNoticeRules')}<small class="${ex.urgent?'couponExpiryUrgent':''}">${escapeHtml(ex.text)}</small></div>`}).join('')}</div><button type="button" class="btn" onclick="this.closest('.couponNoticeOverlay').remove();showAccountCoupons()">Kuponlarımı Gör</button></div>`;
  document.body.appendChild(el)
}
function accountCouponsHtml(){const rows=sortedAccountCoupons();return rows.length?`<div class="accountCouponList">${rows.map(c=>{const ex=couponExpiryInfo(c);return `<div class="accountCouponCard ${c.status!=='active'?'is-muted':''}"><div class="accountCouponMain"><div class="accountCouponCode"><b>${escapeHtml(c.code||'')}</b><button type="button" class="couponCopyBtn" onclick="copyCouponCode('${escapeAttr(c.code||'')}',this)" aria-label="Kupon kodunu kopyala">${couponCopyIcon()}</button></div><span>${escapeHtml(c.headline||c.title||'Kupon')}</span></div><strong>${escapeHtml(couponDiscountLabel(c))}</strong>${couponRulesHtml(c)}<small class="${ex.urgent?'couponExpiryUrgent':''}">${escapeHtml(ex.text)}</small></div>`}).join('')}</div>`:'<div class="accountEmptyState">Şu anda hesabınıza tanımlı kupon bulunmuyor.</div>'}
function renderAccountCouponsPage(){openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Kuponlarım</h2></div>${accountCouponsHtml()}</div>`,'showAccountHome()'))}
async function showAccountCoupons(updateRoute=true){try{document.activeElement?.blur()}catch(_){}if(updateRoute)syncAccountRoute('/hesabim/kuponlar');markCouponNotificationsViewed();renderAccountCouponsPage();try{const r=await apiJson('/api/account/coupons');currentAccountCoupons=Array.isArray(r.coupons)?r.coupons:[];updateCouponNotificationBadges();if(location.pathname==='/hesabim/kuponlar')renderAccountCouponsPage()}catch(e){console.warn('Kuponlar yenilenemedi:',e.message)}}
...
function confirmLogoutAccount(){if(document.querySelector('.accountLogoutConfirm'))return;const el=document.createElement('div');el.className='accountLogoutConfirm';el.innerHTML=`<div class="accountLogoutConfirmCard"><b>Çıkış yapılsın mı?</b><div><button type="button" data-logout-yes>Evet</button><button type="button" data-logout-no>Hayır</button></div></div>`;el.addEventListener('pointerdown',e=>e.stopPropagation(),true);el.addEventListener('click',e=>e.stopPropagation(),true);el.querySelector('[data-logout-yes]').addEventListener('click',()=>{el.remove();requestAnimationFrame(()=>logoutAccount())});el.querySelector('[data-logout-no]').addEventListener('click',()=>{el.remove();requestAnimationFrame(()=>{try{document.activeElement?.blur()}catch(_){}})});document.body.appendChild(el)}
async function logoutAccount(){try{await apiJson('/api/auth/logout',{method:'POST',body:'{}'})}catch{}currentAccountUser=null;currentAccountAddresses=[];currentAccountCoupons=[];currentAccountOrders=[];checkoutState.appliedCouponIds=[];favorites.clear();setAccountHeaderHint(null);updateAccountHeader();updateCouponNotificationBadges();updateFavoriteBadge();renderProducts($('#search')?.value||'');closeDrawer();history.replaceState({...history.state,shazAccount:false},'','/');window.scrollTo({top:0,left:0,behavior:'auto'});centerToast('✓ Başarıyla çıkış yapıldı')}
...
toggleFav=async function(id,e){e?.preventDefault();e?.stopPropagation();if(!currentAccountUser)return showAuthChoice();try{const active=favorites.has(id);await apiJson('/api/account/favorites/'+encodeURIComponent(id),{method:active?'DELETE':'POST',body:active?undefined:'{}'});active?favorites.delete(id):favorites.add(id);updateFavoriteBadge();renderProducts($('#search')?.value||'')}catch(err){alert(err.message)}};
...
// ---------- v164 PWA / footer / cookie preferences ----------
function footerPage(title,body){openDrawer(`<div class="footerInfoPage"><div class="wizardHead"><button class="pill" onclick="closeDrawer()">← Geri</button><h2>${escapeHtml(title)}</h2></div><div class="footerInfoBody">${body}</div></div>`)}
function footerSettingsValue(...keys){for(const k of keys){const v=settings?.[k];if(String(v??'').trim())return String(v).trim()}return ''}
function footerBusinessRows(){const rows=[['Marka / İşletme','SHAZ'],['İşletme sahibi / Unvan',footerSettingsValue('legalName','businessOwner','companyName','ownerName')],['Merkez adresi',footerSettingsValue('businessAddress','companyAddress','address')],['Telefon',footerSettingsValue('businessPhone','phone')||footerSettingsValue('whatsapp')],['E-posta',footerSettingsValue('businessEmail','email')],['KEP',footerSettingsValue('kep','kepAddress')],['VKN / MERSİS',footerSettingsValue('vkn','taxNumber','mersis','mersisNo')]];return rows.filter(([,v])=>String(v||'').trim()).map(([k,v])=>`<div class="footerInfoRow"><b>${escapeHtml(k)}</b><span>${escapeHtml(v)}</span></div>`).join('')}
async function openFooterInfoPage(kind){if(kind==='returns'){await loadPublicLegalDocuments();return openLegalDocument('PRE_INFORMATION')}const pages={about:['Hakkımızda','<p>SHAZ erkek aksesuarları ve kişiselleştirilebilir ürün seçenekleri sunar.</p>'],contact:['İletişim',`<div class="footerInfoRows">${footerBusinessRows()||'<p>İşletmenin zorunlu tanıtıcı bilgileri site ayarlarına henüz tanımlanmamış. Eksik bilgi uydurulmadan boş bırakıldı.</p>'}</div>`],guide:['İşlem Rehberi','<p>Ürünü seçip sepete ekledikten sonra teslimat bilgilerinizi girer, kişiselleştirme bilgilerinizi ve sipariş toplamını kontrol edersiniz. Sipariş onayından önce ürünler, yazı/kişiselleştirme tercihleri, toplam bedel ve teslimat bilgileri yeniden gösterilir.</p><p>Sipariş sırasında gerekli hukuki onaylar kayıtla ilişkilendirilir. Sitede aktif hukuki metinler ilgili bağlantılardan görüntülenebilir. Sipariş verildikten sonra düzeltme gerektiren bir konu varsa müşteri hizmetleriyle iletişim kurulmalıdır.</p>'],order:['Sipariş Sorgulama','<p>Sipariş durumunuzu hesabınız varsa “Hesabım → Siparişlerim” bölümünden takip edebilirsiniz. Hesapsız siparişlerde mevcut kargo/sipariş sorgulama kanallarını kullanabilirsiniz.</p>'],shipping:['Kargo ve Teslimat','<p>Kargo ve teslimat bilgileri sipariş onay ekranında gösterilen bilgiler ve sipariş kaydındaki teslimat adresi esas alınarak yürütülür.</p>'],terms:['Kullanım Koşulları','<p>Siteyi kullanırken verdiğiniz bilgilerin doğru ve güncel olması gerekir. Siparişe ilişkin bağlayıcı koşullar, sipariş sırasında sunulan Ön Bilgilendirme Formu ve Mesafeli Satış Sözleşmesi ile birlikte değerlendirilir.</p>'],commercial:['Elektronik Ticari İleti Aydınlatma Metni','<p>E-posta ve SMS kampanya izinleri üyelik ve hesap ayarlarından ayrı ayrı yönetilebilir. İzni kapattığınızda ilgili kanaldan kampanya, indirim ve fırsat iletileri gönderilmez.</p>']};const p=pages[kind]||pages.about;footerPage(p[0],p[1])}
function initFooter(){document.querySelectorAll('.footerGroupTitle').forEach(btn=>btn.addEventListener('click',()=>{if(matchMedia('(max-width:700px)').matches)btn.closest('.footerGroup')?.classList.toggle('open')}))}
const COOKIE_PREF_KEY='shazCookiePreferencesV1';
function cookiePreferences(){try{return JSON.parse(localStorage.getItem(COOKIE_PREF_KEY)||'null')}catch{return null}}
function saveCookiePreferences(p){try{localStorage.setItem(COOKIE_PREF_KEY,JSON.stringify({...p,updatedAt:new Date().toISOString()}))}catch{}}
function closeCookiePreferences(){document.querySelector('.cookieConsentOverlay')?.remove()}
...
function isStandalonePwa(){return matchMedia('(display-mode: standalone)').matches||window.navigator.standalone===true}
function isAppleMobile(){return /iphone|ipad|ipod/i.test(navigator.userAgent)}
async function subscribeShazPush(reg,publicKey){let sub=await reg.pushManager.getSubscription();if(!sub)sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:base64UrlToUint8Array(publicKey)});await apiJson('/api/push/subscribe',{method:'POST',body:JSON.stringify(sub.toJSON())});return sub}
function removePushPrompt(){document.querySelector('.pushPermissionPrompt')?.remove()}
async function enableShazNotifications(btn){try{btn.disabled=true;const permission=await Notification.requestPermission();if(permission!=='granted'){localStorage.setItem('shazPushPromptDismissedAt',String(Date.now()));removePushPrompt();return}const cfg=await apiJson('/api/push/public-key');if(!cfg.enabled||!cfg.publicKey)throw new Error('Bildirim servisi henüz yapılandırılmamış.');const reg=await navigator.serviceWorker.ready;await subscribeShazPush(reg,cfg.publicKey);removePushPrompt();toast('✓ SHAZ bildirimleri açıldı')}catch(e){alert(e.message||'Bildirimler açılamadı.')}finally{btn.disabled=false}}
function showPushPermissionPrompt(){if(document.querySelector('.pushPermissionPrompt'))return;const el=document.createElement('div');el.className='pushPermissionPrompt';el.innerHTML=`<div><b>SHAZ bildirimlerini aç</b><p>Kampanya, sipariş ve özel fırsat bildirimlerini cihazınızda alabilirsiniz.</p><div><button type="button" class="pill" data-push-later>ŞİMDİ DEĞİL</button><button type="button" class="btn" data-push-enable>BİLDİRİMLERİ AÇ</button></div></div>`;document.body.appendChild(el);el.querySelector('[data-push-later]').onclick=()=>{try{localStorage.setItem('shazPushPromptDismissedAt',String(Date.now()))}catch{}removePushPrompt()};el.querySelector('[data-push-enable]').onclick=function(){enableShazNotifications(this)}}
async function registerShazPwa(){if(!('serviceWorker'in navigator))return;try{const reg=await navigator.serviceWorker.register('/service-worker.js',{scope:'/'});if(!('PushManager'in window)||!('Notification'in window))return;const cfg=await apiJson('/api/push/public-key').catch(()=>null);if(!cfg?.enabled||!cfg.publicKey)return;if(Notification.permission==='granted'){await subscribeShazPush(reg,cfg.publicKey).catch(()=>{});return}if(Notification.permission==='denied')return;if(isAppleMobile()&&!isStandalonePwa())return;let dismissed=0;try{dismissed=Number(localStorage.getItem('shazPushPromptDismissedAt')||0)}catch{}if(Date.now()-dismissed<7*24*60*60*1000)return;setTimeout(showPushPermissionPrompt,700)}catch(e){console.warn('PWA kaydı başarısız:',e.message)}}
...
function showUnusedCouponConfirm(){document.querySelector('.couponSkipConfirm')?.remove();const el=document.createElement('div');el.className='couponSkipConfirm';el.innerHTML=`<div class="couponSkipConfirmCard"><b>İndirim kuponu kullanmadınız</b><p>Hesabınızda kullanılabilir kupon bulunuyor. Kullanmak ister misiniz?</p><div><button type="button" onclick="this.closest('.couponSkipConfirm').remove();openCheckoutCouponPanel()">Evet</button><button type="button" onclick="this.closest('.couponSkipConfirm').remove();checkoutState.couponSkipConfirmed=true;proceedToAddressWithUpsellConfirmed()">Hayır</button></div></div>`;document.body.appendChild(el)}
