...
function positionSavedAddressSuggestions(box,input){if(!box||!input||!document.body.contains(input)||activeSavedAddressSuggestionInput!==input){closeSavedAddressSuggestions();return}const r=input.getBoundingClientRect(),vv=window.visualViewport,gap=6,viewportTop=Math.max(0,vv?.offsetTop||0),viewportH=Math.max(180,vv?.height||window.innerHeight),viewportBottom=viewportTop+viewportH,width=Math.max(160,Math.min(r.width,window.innerWidth-16)),spaceBelow=Math.max(52,viewportBottom-r.bottom-gap-6),available=Math.max(52,Math.min(210,spaceBelow));box.style.width=width+'px';box.style.left=Math.max(8,Math.min(r.left,window.innerWidth-width-8))+'px';box.style.maxHeight=available+'px';box.style.top=Math.round(r.bottom+gap)+'px';box.classList.remove('is-above')}
...
function showAuthChoice(trigger='account'){
  ...
  document.body.appendChild(el);const anchor=document.getElementById(trigger==='favorites'?'favLink':'accountBtn');if(anchor&&matchMedia('(min-width:701px)').matches){const r=anchor.getBoundingClientRect();el.style.left=Math.max(12,Math.min(innerWidth-el.offsetWidth-12,r.left+r.width/2-el.offsetWidth/2))+'px';el.style.top=(r.bottom+8)+'px';el.style.right='auto'}setTimeout(()=>document.addEventListener('pointerdown',accountPopoverOutside,{once:true}),0)
}
