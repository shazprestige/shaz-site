...
function announcementSignature(cfg={}){
  const raw=[cfg.eyebrow||'',cfg.title||'',cfg.text||'',cfg.buttonText||'',cfg.fontScale||100].join('|');
  let h=2166136261;
...
function renderSiteAnnouncement(force=false){
  ...
  const scale=Math.max(80,Math.min(130,Number(cfg.fontScale)||100));
  wrap.style.setProperty('--announcement-scale',String(scale/100));
