...
function focusDrawerField(el){
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    if(el.id?.startsWith('addr-')){el.scrollIntoView({block:'center',behavior:'smooth'});setTimeout(repositionSavedAddressSuggestions,180)}else if(r.top<topGuard||r.bottom>vh-bottomGuard)el.scrollIntoView({block:'center',behavior:'smooth'});
  }));
}
...
function addrInputInner(label,id,value='',type='text',placeholder=''){
  const suggest=currentAccountUser&&checkoutState.manualAddressFromSaved?` data-address-suggest="${escapeAttr(id)}" onfocus="queueSavedAddressSuggestions(this,'${escapeAttr(id)}')" onclick="queueSavedAddressSuggestions(this,'${escapeAttr(id)}')" oninput="updateSavedAddressSuggestions(this,'${escapeAttr(id)}')"`:'';
...
async function showAccountOrders(updateRoute=true){try{document.activeElement?.blur()}catch(_){}openDrawer(accountStandardPage(`<div class=\"accountShell accountCenteredSubPage accountTopSubPage\"><div class=\"wizardHead\"><button class=\"pill\" onclick=\"showAccountHome()\">← Geri</button><h2>Siparişlerim</h2></div><div class=\"accountEmptyState accountLoadingState\">Yükleniyor…</div></div>`,'showAccountHome()'));if(updateRoute)syncAccountRoute('/hesabim/siparisler');try{const r=await apiJson('/api/account/orders');const rows=(r.orders||[]).map(o=>`<div class="accountOrder"><div class="accountOrderTop"><b>Sipariş #${escapeHtml(o.dailyDisplayId||o.id)}</b><span class="accountOrderTopTotal">${money(o.total)}</span></div><small>${escapeHtml(o.createdAtTR||o.createdAt||'')}</small>${accountOrderThumbs(o.items||[])}<div class="accountOrderListStatus">${escapeHtml(accountOrderStatusLabel(o.status))}</div><div class="accountRowActions"><button class="smallBtn" onclick="showAccountOrderDetail('${escapeAttr(o.id)}')">Detay</button></div></div>`).join('')||'<div class="accountEmptyState">Henüz hesabınıza bağlı sipariş bulunmuyor.</div>';openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage accountTopSubPage"><div class="wizardHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Siparişlerim</h2></div>${rows}</div>`,'showAccountHome()'))}catch(e){alert(e.message)}}
...
async function showAccountAddresses(updateRoute=true){
  try{document.activeElement?.blur()}catch(_){}
  openDrawer(accountStandardPage(`<div class="accountShell accountCenteredSubPage accountTopSubPage"><div class="wizardHead accountAddressHead"><button class="pill" onclick="showAccountHome()">← Geri</button><h2>Adreslerim</h2><button class="accountNewAddressTop" type="button" onclick="showAddressForm('')">+ Yeni Adres</button></div><div class="accountEmptyState accountLoadingState">Yükleniyor…</div></div>`,'showAccountHome()'));
